...
| Persistenz | ESP-IDF NVS / `nvs_flash` (eingebaut) | Bestandteil ESP-IDF 6.0.2; konkrete Adapter-API im Spike zu Issue #90 pruefen | Apache-2.0 (ESP-IDF) | `FRAMEWORK_CANDIDATE`, `SPIKE_REQUIRED` | durch ADR-016 festgelegtes produktives Backend fuer `IStateStore`; keine offene Produktauswahl mehr; Adapterimplementierung und reale Verifikation in Issue #90 |
