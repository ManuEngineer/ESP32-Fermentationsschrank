...
| Persistenz | ESP-IDF NVS / `nvs_flash` (eingebaut) | Bestandteil ESP-IDF 6.0.2; konkrete Adapter-API im Spike zu Issue #90 pruefen | Apache-2.0 (ESP-IDF) | `FIRST_EVALUATION_CANDIDATE`, `SPIKE_REQUIRED`, `FINAL_SELECTION_PENDING` | verbindlicher produktiver Backendkandidat fuer `IStateStore` gemaess ADR-016; Adapterimplementierung und reale Verifikation offen in Issue #90 |
