...
| 3 | Issue #119 – produktive StateStore-/Application-Composition anbinden und real verifizieren | `STEP_2_VERIFICATION_FAILED_BOOT_WDT_BEFORE_RECOVERY`; Schritt 1 gemaess R1.0-Plan umgesetzt und ownerreviewt. Ursachenanalyse (PR #120) real durchgefuehrt: `CAUSE_CLASS=#120_BINARY_OR_INTEGRATION_DEPENDENT`, `ROOT_CAUSE_DIRECTION=NOT_CONTENT_ONLY`. A/B mit byteidentischem `state_store`-Inhalt und identischer sdkconfig/Partitionstabelle: #118 (`fd7e4e3`) 3/3 `application: ready`, #120 9/9 `Interrupt wdt timeout` (inkl. mit geloeschtem `state_store`). Statischer Befund: `app_main()`-Stackframe waechst von 112 auf 16.432 Byte (#118 zu #120) bei unveraendertem `CONFIG_ESP_MAIN_TASK_STACK_SIZE=3584`; realer Panic-Backtrace zeigt bis zu 17.616 Byte Stackverbrauch im main_task-Aufrufpfad, SP faellt bis in den `.dram0.data`-Bereich (spi_flash/esp_ipc_isr/panic/log/heap_caps-Globals). `MATERIAL_ARCHITECTURE_DECISION_OPEN=YES` (Stackgroesse erhoehen vs. Kompositionswurzel restrukturieren). Details in PR #120. Plan `docs/tasks/issue-119-platform-application-composition-plan.md @ 5effae0d8b3ec82abe5e3864082b1ce237f8f03f`, Draft-PR #120, keine technische Korrektur und kein Schritt 2-Fortgang ueber diesen Befund hinaus. | STOP – Ownerentscheidung zwischen Stackgroessen-Erhoehung und Restrukturierung der Kompositionswurzel; kein Backendwechsel, keine Korrektur ohne Freigabe, keine weiteren Realgates. |
