...
|---:|---|---|---|
| 0 | Issue #164 – R1-WLAN und lokale Touch-Netzwerkseite | `BASE_MAIN=b871375f494701bed1834013cfeb789856983e3a`; `PR165=MERGED @ 1f1755e5e706fb668472920545b5302fcef1df16`; `PREVIOUS_APPROVED_PLAN_SHA=67fbc1786b42f8ca3dc0afe65cdb4343aed2d70e`; `COMPLETION_PLAN=docs/tasks/issue-164-local-network-touch-completion-plan.md`; `OWNER_DECISION=VARIANT_B_QR_RETAINED`; `R1_BROWSER_HOME_WIFI_SETUP=RETAINED`; `R1_TOUCH_HOME_WIFI_CREDENTIAL_ENTRY=DEFERRED`; `R1_TOUCH_WIFI_KEYBOARD=DEFERRED`; `R1_WLAN_QR_TO_JOIN_SOFTAP=REQUIRED`; `R1_WEBSITE_QR=DEFERRED`; `COMPLETION_PLAN_STATUS=INDEPENDENT_PLAN_FIX_VERIFICATION_REQUIRED`; `PRODUCT_IMPLEMENTATION=NOT_STARTED`; `ISSUE164=OPEN`; `HARDWARE_CLIENT_EVIDENCE=NOT_RUN_PENDING_HARDWARE`; `ACTUATOR_RELEASE=NO`. | Independent Plan Fix Verification des korrigierten exakten Plans; danach lokale Touch-/SoftAP-UI-Slices und der kleine WLAN-QR-Slice. Reale Kamera-/Client-Evidence bleibt bis zum finalen #164-Abschluss separat offen. |
...
Der aktuelle Follow-up-Scope für #164 ist die lokale Touch-Netzwerkseite samt lokaler Anzeige der aktiven SoftAP-Zugangsdaten und dem WLAN-QR zum SoftAP-Beitritt. Die Ownerentscheidung `VARIANT_B_QR_RETAINED` deferiert nur die lokale HOME_WIFI-Credentialeingabe samt Bildschirmtastatur; der browserbasierte Setup-Pfad und der WLAN-QR bleiben R1, ein separater Webseiten-QR bleibt Future Scope. Der versionierte Plan liegt auf `BASE_MAIN=b871375f494701bed1834013cfeb789856983e3a`; `COMPLETION_PLAN_STATUS=INDEPENDENT_PLAN_FIX_VERIFICATION_REQUIRED`, `PRODUCT_IMPLEMENTATION=NOT_STARTED`.
