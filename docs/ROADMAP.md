...
| 1 | Issue #27 – Web/API/Auth auf aktuellem `main` neu aufsetzen | `BASE_SHA=b871375f494701bed1834013cfeb789856983e3a`; `BRANCH=agent/issue-27-web-api-auth-main-restart`; `NEW_PR=DRAFT`; `PLAN_STATUS=OWNER_PLAN_REVIEW_REQUIRED`; `IMPLEMENTATION_AUTHORIZATION=NO`; `OLD_PR167=SUPERSEDED_REFERENCE_ONLY`; `KDF_ALGORITHM=PBKDF2_HMAC_SHA256`; `KDF_WORK_FACTOR=10000`; `KDF_WORK_FACTOR_FALLBACK=NONE`; `ACTUATOR_RELEASE=NO` | Neue vollstaendige Delta-Planrevision unabhaengig pruefen und exakte Plan-SHA freigeben; bis dahin keine Produktimplementation |
