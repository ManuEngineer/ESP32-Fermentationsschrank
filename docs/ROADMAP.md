...
| 2 | Issue #90 – produktiver ESP-IDF-NVS-Adapter fuer `IStateStore` | `R5_9_SLICE_5_BLOCKED_PENDING_OWNER_REVIEW`; PR #118 bleibt OPEN/Draft. Slice 5 fuehrt den einzigen generischen `NvsStateStore`-Adapter, den v6.0.2-Read-/Write-Vertrag, den actor-free Owning-Context-Lifecycle fuer `state_store`/`fermentation`, getrennte 4-MB-Produkt-/Testlayouts, einen transitiv pruefbaren ESP-IDF-Consumer und einen projektspezifischen Host-Harness ueber die offizielle v6.0.2-BDL-API ein. Native/ESP-IDF-Profile, Static Analysis, Layoutpruefung und Regressionen sind PASS; Clean-Linux-BDL und der separate oeffentliche Consumer-Build sind ohne Shim PASS (`14/14` Hosttests). Der Hostlauf weist echte `rh0`-Zielwrites, `old_head_bytes_ne_new_head_bytes`, Write-/Erase-/Sync-Ereignisse, unmittelbare Cut-Baselines und Early-/Late-Artefakte nach. Ein semantisch passender Product-Bridge-Fall ist `PASS`; die beiden BDL-I/O-Failure-Faelle bleiben `NOT_RUN`, weil der Fehlerpfad keinen exakten neuen Head exponiert. Das korrigierte NVS-Modell weist `14352` technische Entries/`114` Seiten/`0x72000` Bytes sowie `4` GC-/Fragmentierungsreserve- und `64` Wear-Headroom-Seiten aus; geplant sind `182` Seiten/`0xB6000` und damit `74` Seiten Headroom in der unveraenderten `0x100000`-Partition. Die BDL-Matrix bleibt `PARTIAL`, `exhaustive=false`; GC-/Erase-Aktivitaet ist `OBSERVED` (14072 Writes/503 Erases), ein GC-/Erase-Cut bleibt `NOT_RUN`, und ein abrupter Power-Cut-Harness ist `POWER_CUT_HARNESS_BLOCKED`. Es gibt keinen `POWER_CUT_PASS`. Callback 12 bleibt `FAIL_CALLBACK_12_NOT_FOUND`, `KNOWN_BACKEND_LIMITATION`, `REAL_NVS_ONLY`, `product_recovery_gate=NOT_RUN`. `SOURCE_OF_TRUTH_CONFLICT: NONE`; Oracle-Luecken: `NONE`; `PRODUCT_RECOVERY_MISMATCH: NONE`. | Ownerreview der vollstaendigen Slice-5-Evidenz bzw. Schliessung oder explizite Behandlung aller genannten Blocker; danach keine Slice-6-/Slice-7-Arbeit ohne separate Freigabe |
