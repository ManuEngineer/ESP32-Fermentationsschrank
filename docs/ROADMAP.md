...
| 2 | Issue #90 – produktiver ESP-IDF-NVS-Adapter fuer `IStateStore` | `IMPLEMENTATION_IN_PR; PLAN_REVISION_PENDING_OWNER_APPROVAL; IMPLEMENTATION_CORRECTIONS_PENDING; HARDWARE_BLOCKED/NOT_RUN`; Draft-PR #117 enthaelt die konsolidierte Planrevision `docs/tasks/issue-90-esp-idf-nvs-adapter-plan.md @ da693e8a24735ff2cc09f019b119083f3792882e` und bleibt gestapelt auf `agent/issue-29-esp32-bringup-plan @ c4c8b33f4dbaef727200ea410d887ec5417aa1b0`. Die bestehende Implementation ist fuer Reviewkorrekturen eingefroren; die Revision korrigiert Kapazitaetsarithmetik und anwendungsneutrale `NvsStateStoreConfig` einschliesslich exakter Write-/Read-Fehlerstatus und legt vollstaendige 4-Byte-Exhaustive-Cuts, bounded No-PSRAM-Harness-Speicher, kalibrierte Power-Cut-Fenster, Clean-/Recoverytrennung, Reboot-Baseline, exakte Run-Key-Driftpruefung, Harness-Dependency-Isolation, Scan-vor-Upload, Hardware-Artefakte sowie getrennte Host-BDL-Erase-Callbacks und reale Raw-Page-/Readback-Evidenz fest. Build-/Source-Provenienz verlangt nun Remote-/GitHub-Aufloesung im PR-Verlauf; die historische R3-Bezeichnung ist entfernt. `state_store` / `fermentation` sowie 69 Seiten / 276 KiB bleiben konkrete R1-Werte, keine universellen Adapterdefaults. Fruehere Host-/Build-PASS sind bis zur Phase-2-Korrektur kein aktueller Abschlussnachweis. #29 bleibt offen und blockiert die Realflash-, Partitions-, Power-Cut-, Readback-, Ressourcen- und Wear-Nachweise; kein zweiter Persistenzport und keine neue Persistenzarchitektur. | Ownerfreigabe der exakten konsolidierten Plan-SHA; danach Phase-2-Korrekturen und gezielte Nachweise |
