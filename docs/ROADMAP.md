...
| 2 | Issue #90 – produktiver ESP-IDF-NVS-Adapter fuer `IStateStore` | `R5_9_SLICE_5_BLOCKED_PENDING_OWNER_REVIEW`; PR #118 bleibt OPEN/Draft. Slice 5 fuehrt den einzigen generischen `NvsStateStore`-Adapter, die v6.0.2-Read-/Write-Vertraege, getrennte 4-MB-Produkt-/Testlayouts und einen offiziellen BDL-Hostnachweis ein. Native/ESP-IDF-Profile, Static Analysis und Regressionen sind PASS; der saubere Linux-BDL-Hostlauf bleibt wegen fehlendem `libbsd-dev` BLOCKED und wird nicht durch den temporaeren `/tmp`-Kompatibilitaetsshims-Lauf als Clean-PASS ausgegeben. Die BDL-Matrix bleibt `PARTIAL`, `exhaustive=false`; Callback 12 bleibt `FAIL_CALLBACK_12_NOT_FOUND`, `KNOWN_BACKEND_LIMITATION`, `REAL_NVS_ONLY`, `product_recovery_gate=NOT_RUN`. `SOURCE_OF_TRUTH_CONFLICT: NONE`; Oracle-Luecken: `NONE`. | Ownerreview des Slice-5-Ergebnisses und Entscheidung zum fehlenden Clean-Host-BDL-Nachweis; danach keine Slice-6-/Slice-7-Arbeit ohne separate Freigabe |
