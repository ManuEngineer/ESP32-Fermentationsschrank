...
| 2 | Issue #90 – produktiver ESP-IDF-NVS-Adapter fuer `IStateStore` | `R5_9_SLICES_1_TO_6_IMPLEMENTED_AND_VERIFIED`; `R5_9_SLICE_7_PARTIALLY_EXECUTED_BLOCKED_BY_PRODUCT_COMPOSITION`. Kanonischer Plan bleibt R5.9 (`docs/tasks/issue-90-clean-restart-plan-r5.9.md @ baf0b2ae04cd42afa75dfa00e21d900116b38bc8`); keine weitere #90-Planrevision. Real erbracht: `SOFTWARE_ORACLE_GATE=PASS`, `NVS_ADAPTER_GATE=PASS`, `CAPACITY_GATE=PASS`, `DEFINED_BACKEND_MATRIX_COMPLETE=true`, `CI_ARTIFACT_SECRET_SCAN_GATE=PASS`, `STATIC_PROJECT_SCRATCH_EVIDENCE=PASS`, `CONFIGURED_RELEASE_MAIN_TASK_STACK=3584`, `BOARD_IDENTITY_GATE=PASS`, `UART_RESET_GATE=PASS` (reales Board, zwei unabhaengige Reboots). Offen, weil der echte Produktpfad noch nicht komponiert ist: `REAL_NVS_RECOVERY_GATE=NOT_RUN`, `CALLBACK_12_REAL_NVS_PRODUCT_GATE=BLOCKED`, `MANUAL_POWER_CUT_GATE=NOT_RUN`, `FULL_RUNTIME_STACK_HEADROOM=BLOCKED/NOT_RUN_FOR_FINAL_RECOVERY_WORKLOAD`. `device_platform -> fermentation_app`, `device_platform_esp_idf -> fermentation_app` und fermentationsspezifische `NvsStateStore`-Defaults bestaetigt `NONE`. `SOURCE_OF_TRUTH_CONFLICT: NONE`; `Oracle-Luecken: NONE`; `PRODUCT_RECOVERY_MISMATCH: NONE`. Der separat ownerfreizugebende Composition-Schritt aus R5.9 Punkt 9 wird als Issue #119 gefuehrt. | Ownerreview von PR #118 auf dem eingefrorenen technischen Stand; Slice-7-Restgates warten auf die #119-Composition. |
| 3 | Issue #119 – produktive StateStore-/Application-Composition anbinden und real verifizieren | `STEP_1_IMPLEMENTED_PENDING_OWNER_REVIEW`; Schritt 1 des separaten Composition-Schritts gemaess R5.9 Punkt 9 umgesetzt: produktiver `EspTimeZoneResolver`, reale Configuration-/Run-Persistence-Composition und SafetyCore-Projektion. Plan `docs/tasks/issue-119-platform-application-composition-plan.md @ 5effae0d8b3ec82abe5e3864082b1ce237f8f03f`, Draft-PR #120, Schritt 2 nicht freigegeben und nicht gestartet. | Owner Full Review von Schritt 1; danach separates Owner-Gate fuer Schritt 2. |
