...
| 2 | Issue #90 – produktiver ESP-IDF-NVS-Adapter fuer `IStateStore` | `IMPLEMENTATION_IN_PR; IMPLEMENTATION_CORRECTIONS_IN_PROGRESS; CI_REGRESSION_PASS; EXHAUSTIVE_FAILED_CALLBACK_12; HARDWARE_BLOCKED/NOT_RUN`; Draft-PR #117 enthaelt die ownerfreigegebene Planrevision `docs/tasks/issue-90-esp-idf-nvs-adapter-plan.md @ da693e8a24735ff2cc09f019b119083f3792882e` und bleibt gestapelt auf `agent/issue-29-esp32-bringup-plan @ c4c8b33f4dbaef727200ea410d887ec5417aa1b0`. Die bestehende Implementation wurde nach diesem Plan korrigiert: anwendungsneutrale `NvsStateStoreConfig` mit exakten invalid-config-Status, korrigierte Kapazitätsarithmetik, vollständige 4-Byte-Host-Cuts, bounded No-PSRAM-Harness-Scratch, kalibrierte Power-Cut-Fenster, Clean-/Recoverytrennung, Reboot-Baseline, exakte Run-Key-Driftprüfung, Harness-Dependency-Isolation, Scan-vor-Upload, Hardware-Artefakte sowie getrennte Host-BDL-Erase-Callbacks und reale Raw-Page-/Readback-Evidenz. Der vollständige Hostlauf bleibt wegen des reproduzierbaren `NotFound`-Befunds am internen Entry-State-Callback 12 `FAILED`; deshalb wird kein Exhaustive- oder Host-Abschluss-PASS behauptet. Build-/Source-Provenienz verlangt Remote-/GitHub-Auflösung im PR-Verlauf. `state_store` / `fermentation` sowie 69 Seiten / 276 KiB bleiben konkrete R1-Werte, keine universellen Adapterdefaults. #29 bleibt offen und blockiert Realflash-, Partitions-, Power-Cut-, Readback-, Ressourcen- und Wear-Nachweise; kein zweiter Persistenzport und keine neue Persistenzarchitektur. | Exhaustive-Callback-12-Befund klären, danach gezieltes Ownerreview; reale Hardware bleibt blockiert |
