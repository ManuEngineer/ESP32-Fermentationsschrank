...
| 4 | Issue #121 – Release-1 Device-/Application-Lifecycle- und Safety-Policy-Vereinfachung | `IMPLEMENTATION_STEP_8=BLOCKED`; freigegebener Plan `docs/tasks/issue-121-lifecycle-safety-simplification-plan.md @ 1568c8930aeb759468aeca1e27da93889875554a`; Schritte 1–7 PASS und ownerreviewt; `STEP_8_STACK_CORRECTION_SOURCE_SHA=2cedf885e0916df784764ff050fa54b96e1aec1c`; `STEP_8_STATIC_ANALYSIS_SOURCE_SHA=2cedf885e0916df784764ff050fa54b96e1aec1c` (erweiterte GNU-only Instrumentierung); der vollständige Configuration-Bootpfad ist im frischen GNU-Callgraph nachgewiesen, aber über Budget: `PRODUCT_BOOT_CONFIG_RECOVERY_MAX_PATH=10544 B` (Bring-up) bzw. `10576 B` (Release), `PRODUCT_BOOT_CUMULATIVE_STACK_PATH=10576 B`, Headroom `-6992 B` bei `CONFIGURED_RELEASE_MAIN_TASK_STACK=3584 B`; Witness `app_main(144)->FermentationApplication::begin(368)->ConfigurationRecoveryService::boot(2400)->continueEpochBuild(1648)->executeInitialGraph(1440)->loadCanonicalGraph(1712)->loadBranch(448)->loadBranchParts(752)->decodeProgramCatalogPayload(256)->validateProgramCatalog(864)->FactoryProgramCatalog::programs(256)->makeFactoryProgram(256)`; alle Witness-Frames sind `static` und die geprüften direkten `.ci`-Kanten vorhanden; `NO_PRODUCT_REACHABLE_SINGLE_FRAME_EXCEEDS_CONFIGURED_TASK_STACK=PASS`; `PRODUCT_REACHABLE_STATIC_STACK_GATE=FAIL`. Die zuvor gemessenen Run-/Discard-Teilpfade bleiben als Teilnachweis erhalten, sind wegen des Boot-FAILs aber kein Gesamtgate. `ESP_IDF_BRINGUP_BUILD=PASS`, `ESP_IDF_RELEASE_BUILD=PASS`, `ESP_CLANG_BRINGUP=NOT_RUN_STACK_GATE_FAIL`, `ESP_CLANG_RELEASE=NOT_RUN_STACK_GATE_FAIL`, `ESP_IDF_NVS_ADAPTER_HOST=NOT_RUN_STACK_GATE_FAIL`, `ARCHITECTURE_BOUNDARIES=PASS`, `SECRET_SCAN=PASS`, `CLANG_FORMAT=PASS` und `GIT_DIFF_CHECK=PASS`; `PRODUCT_FRESH_START_CUMULATIVE_STACK_PATH=NOT_PRODUCT_REACHABLE`; `PRODUCT_RESUME_CUMULATIVE_STACK_PATH=NOT_PRODUCT_REACHABLE`; `OWNER_HARDWARE_RUN_AUTHORIZATION=NO`; `MAIN_TASK_STACK_HIGH_WATERMARK=NOT_RUN`; `HEAP_LARGEST_BLOCK_RUNTIME_GATE=NOT_RUN`; `HARDWARE_RUN=NO`; `PR120=OPEN_DRAFT_FROZEN`; `PR122=OPEN_DRAFT`; `ISSUE121=OPEN`; `MERGE=NO`; `PLAN_CHANGE=NO`; `TASK_STACK_SIZE_INCREASE=NO`; `WIRE_FORMAT_CHANGE=NO`; `SCHEMA_CHANGE=NO`; `PLAN_DEVIATION=NONE`; `SOURCE_OF_TRUTH_CONFLICT=NONE`. | STOP – statischer Produkt-Stackpfad FAIL; exakte Configuration-Callchain an Owner. Keine weitere Refactor-Runde, keine Clang-/NVS-Nachgates, kein Hardwarelauf, kein Main-Task-HWM und kein Merge durch den Agenten. |
