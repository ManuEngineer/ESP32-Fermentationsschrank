...
| 2 | Issue #90 – produktiver ESP-IDF-NVS-Adapter fuer `IStateStore` | `ISSUE90=OPEN`; `PR128=DRAFT`; `CURRENT_R1_DEVELOPMENT_BASE=integration/r1-development`; `PR123=MERGED`; kanonischer Korrekturplan: `docs/tasks/issue-90-slice7-host-utc-seam-correction-plan.md @ cdb19e52479db8a0db83270811c73910842c9393` (KISS, Owner-freigegeben); `IMPLEMENTATION=DIGITAL_PASS` (Commits `210d226`,`79add23`,`5bb7525`,`0dba45d`,`fb18fad`,`ecd9145`,`7b7522b`: gemeinsame `EspTimerTimeSource`, `SET_TRUSTED_UTC`-Harness-Befehl, `RecoveryEvaluation` in Runner-Oracle, esptool-Reset-/Versionsfixes, #29-Probe-Isolation vom Harness-Build, Reset-/Bootloader-Dokumentation korrigiert, UART-RX-Driver-Install-Fix); `ISSUE90_UART_RX_SMOKE=PASS` (real verifiziert: `ISSUE90_UART_RX_READY=PASS`, `ISSUE90_READY` mit korrekter Source-SHA, kein #29-Probe-Log, STATUS-Kommando beantwortet, `actuator_release=false`, 35s stabil ohne Panic/Watchdog/Reset); `HARDWARE_CAMPAIGN=CAMPAIGN2_READY_FOR_OWNER_POWER_CUT_APPROVAL` (Production-Rebaseline auf `0efd00c` real durchgefuehrt und Owner-reviewed; frischer zweiter Kampagnenzyklus auf demselben Source-Stand `0efd00c` mit neuen, vorher nicht existierenden Pfaden `build/issue90_slice7_hardware/campaign2_pre_harness_backup`/`campaign2_pre_harness_manifest.json`: `PRE_HARNESS_BACKUP=PASS`, `RELEASE_ARTIFACTS_FROZEN=YES`, Harness auf exakt `0efd00c` gebaut/geflasht, Isolationsgate PASS (`issue_29_bringup`-Probe-Symbole im Harness-Build abwesend, im normalen `esp32_bringup`-Build weiterhin vorhanden), UART-/Stabilitaets-Smoke real wiederholt: `ISSUE90_UART_RX_READY=PASS`, `ISSUE90_READY` mit exakt passender Source-SHA, kein #29-Probe-Log, STATUS beantwortet, `actuator_release=false`, 35s stabil ohne Panic/Watchdog/unerwarteten Reset; keine Power-Cut-Kommandos armiert); `REAL_POWER_CUTS=NOT_RUN`; `POWER_CUT_APPROVAL=NOT_YET`; historischer R5.9-Plan bleibt Grundlage: `docs/tasks/issue-90-clean-restart-plan-r5.9.md @ baf0b2ae04cd42afa75dfa00e21d900116b38bc8`. | Owner-Freigabe der sechs realen Power-Cuts (3x config, 3x run, actor-free) auf dem zweiten Kampagnenzyklus. |
