...
| 2 | Issue #90 – produktiver ESP-IDF-NVS-Adapter fuer `IStateStore` | `IMPLEMENTATION_BLOCKED_PENDING_R5_1_PLAN_APPROVAL`; Draft-PR #117 wartet auf Ownerfreigabe der exakten R5.1-Plan-SHA. Der vollständige Host-`exhaustive`-Lauf bleibt am reproduzierten Callback 12 / `NotFound` `FAIL`; `f0c7d9b6...` ist bereits im gepinnten v6.0.2-Verhalten enthalten und kein offener Lösungs-Kandidat. Schritt 1A ist nur Analyse/Evidenz ohne Dependency- oder Vendor-Patchänderung; bei belastbarem Herstellerpfad folgt ein separates Owner-Subgate, sonst `BLOCKED`. Das #29-Pegel-Restgate blockiert die zulässigen aktorfreien NVS-Tests nicht; Power-Cut bleibt an ein ownerverifiziertes kontrollierbares Fixture gebunden. | Exakte R5.1-Plan-SHA freigeben; danach Schritt 1A/Owner-Subgate/1B, Host-Exhaustive und erst dann reale #90-NVS-Nachweise |
