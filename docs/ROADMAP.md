...
|---:|---|---|---|
| 0 | Issue #164 – R1-WLAN und lokale Touch-Netzwerkseite | `BASE_MAIN=b871375f494701bed1834013cfeb789856983e3a`; `PR165=MERGED @ 1f1755e5e706fb668472920545b5302fcef1df16`; `PREVIOUS_APPROVED_PLAN_SHA=67fbc1786b42f8ca3dc0afe65cdb4343aed2d70e`; `OWNER_APPROVED_PLAN_SHA=931db488125a6e7eca76b1e8bb88ffc98b97a073`; `COMPLETION_PLAN=docs/tasks/issue-164-local-network-touch-completion-plan.md`; `COMPLETION_PLAN_SHA=a514544321807ad47abce5de27a68595085079d3`; `OWNER_DECISION=VARIANT_B_QR_RETAINED`; `R1_BROWSER_HOME_WIFI_SETUP=RETAINED`; `R1_TOUCH_HOME_WIFI_CREDENTIAL_ENTRY=DEFERRED`; `R1_TOUCH_WIFI_KEYBOARD=DEFERRED`; `R1_WLAN_QR_TO_JOIN_SOFTAP=REQUIRED`; `R1_WEBSITE_QR=DEFERRED`; `COMPLETION_PLAN_STATUS=IMPLEMENTATION_COMPLETE_PENDING_INDEPENDENT_REVIEW`; `IMPLEMENTATION_HEAD=2e31e1ca00df13180e3722237107fd4ed227f351`; `PRODUCT_IMPLEMENTATION=COMPLETE_DIGITAL`; `PRODUCT_TESTS=PASS_TARGETED_NATIVE`; `BUILDER_STATIC_ANALYSIS_SELF_CHECK=PASS`; `ESP32_BRINGUP_BUILD=PASS`; `ESP32_RELEASE_BUILD=PASS`; `ISSUE164=OPEN`; `PR171=OPEN_DRAFT`; `HARDWARE_CLIENT_EVIDENCE=NOT_RUN_PENDING_HARDWARE`; `ACTUATOR_RELEASE=NO`. | Independent implementation review/Fix Verification auf dem aktuellen Draft-HEAD. Der reale Kamera-/Client-Scan des Produktdisplays bleibt `NOT_RUN` und ist vor dem finalen Abschluss von #164 erforderlich. |
...
| 2 | Issue #27 / PR #170 – Web/API/Auth | `ISSUE27=OPEN`; `CURRENT_PR27=170`; `PR170=OPEN_DRAFT`; `PR167=SUPERSEDED_REFERENCE_ONLY`; `CONSUMER_PREDECESSOR=ISSUE168`; `ACTUATOR_RELEASE=NO`. | PR #170 auf aktuellem `main` und gegen den gemergten #168-Vorgänger revalidieren; Owner-Gates bleiben bestehen |
...
PR #165 ist auf `main` unter `1f1755e5e706fb668472920545b5302fcef1df16` gemergt. Issue #164 bleibt bis zur realen Hardware-/Client-Evidence offen.
Der lokale Touch-/SoftAP-Anzeigepfad und der WLAN-QR sind digital implementiert und gezielt getestet; der Builder-Self-Check sowie die ESP-IDF-Profile `bringup` und `release` sind PASS. Die Ownerentscheidung `VARIANT_B_QR_RETAINED` deferiert weiterhin nur die lokale HOME_WIFI-Credentialeingabe samt Bildschirmtastatur; browserbasierter Setup-Pfad und WLAN-QR bleiben R1, ein separater Webseiten-QR bleibt Future Scope. Der freigegebene Scope-Plan ist `OWNER_APPROVED_PLAN_SHA=931db488125a6e7eca76b1e8bb88ffc98b97a073`; die aktuelle Evidence-Fassung liegt unter `COMPLETION_PLAN_SHA=a514544321807ad47abce5de27a68595085079d3`. `COMPLETION_PLAN_STATUS=IMPLEMENTATION_COMPLETE_PENDING_INDEPENDENT_REVIEW`, `PRODUCT_IMPLEMENTATION=COMPLETE_DIGITAL`. Issue #164 und PR #171 bleiben offen/Draft bis zur unabhängigen Implementation-Fix-Verification; der reale Kamera-/Client-Scan ist vor Issueschliessung nachzuholen.
