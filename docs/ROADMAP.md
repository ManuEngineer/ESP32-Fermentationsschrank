...
| 2 | Issue #90 – produktiver ESP-IDF-NVS-Adapter fuer `IStateStore` | `IMPLEMENTATION_BLOCKED_PENDING_R5_4_PLAN_APPROVAL`; Draft-PR #117 wartet auf Ownerfreigabe der exakten R5.4-Plan-SHA `c35ce0342898f0e19d3cce5e6a7eaa077f73bad6`. Der vollständige Host-`exhaustive`-Lauf bleibt am reproduzierten Callback 12 / `NotFound` `FAIL`; `f0c7d9b6...` ist bereits im gepinnten v6.0.2-Verhalten enthalten und kein offener Lösungs-Kandidat. Die allgemeine R5.4-Freigabe autorisiert ausschließlich Phase A. Danach folgen eigene Owner-Subgates für Host-Evidence-Foundation, Herstellervergleich, exaktes Herstellerziel, jeden R6-Slice, Final Verification, Board und Power-Cut. OTA bleibt optionale Zukunft ohne #90-Reserve; das #29-Pegel-Restgate blockiert sichere aktorfreie NVS-Tests nicht. | Exakte R5.4-Plan-SHA freigeben; danach Phase A und je Folgeschritt das konkret benannte Owner-Gate abwarten |
