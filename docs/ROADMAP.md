...
| 6 | Issue #126 – R1-Absolute-Zeitplattform mit DS3231M und ESP-IDF-SNTP | `PLAN_ONLY`; `IMPLEMENTATION=NOT_STARTED`; `MERGE=NO`; `OWNER_PLAN_REVIEW_REQUIRED=YES`; `OWNER_DECISIONS_REQUIRED=NONE`; `RTC_DEVICE=DS3231M`; `RTC_FAMILY=DS3231_COMPATIBLE`; `RTC_HARDWARE_OPTIONAL=YES`; `NTP_ONLY_MODE_SUPPORTED=YES`; `NEW_PRODUCTIVE_RUN_START_REQUIRES_TRUSTED_UTC=YES`; `R1_CURRENT_FERMENTING_CHECKPOINT_REQUIRES_TRUSTED_UTC=YES`; `PERIODIC_FERMENTING_CHECKPOINT_WITHOUT_TRUSTED_UTC=SKIP_NO_WRITE`; `WRITE_INVARIANT_TIGHTENED=YES`; `DS3231M_LIBRARY_COMPATIBILITY=UNVERIFIED_IMPLEMENTATION_GATE`; `DS3231M_LIBRARY_ADOPTION=PROVISIONAL`; `I2C_PORT_LIFETIME_OWNER=I2CDEV`; `RTC_HARDWARE=BLOCKED_OWNER_HARDWARE_PENDING`; `NTP_REAL_NETWORK_RUN=NOT_RUN`; kanonischer Plan: `docs/tasks/issue-126-absolute-time-rtc-ntp-plan.md @ 332f126fe403e6adab89ec0ba3b60657aaaa5748`; #89 bleibt Connectivity-Eigentümer, #124 bleibt fachlich unverändert. | Owner-Review des exakten Plan-Commits; danach nur mit Freigabe umsetzen. |
...
app-neutrale trusted UTC ueber RTC/NTP und wird nur plan-only bearbeitet; #18
...
- Issue #126 bleibt bis zur Ownerfreigabe des exakten Plan-Commits plan-only;
  seine RTC-/NTP-Implementierung darf #89-Connectivity nicht duplizieren und
  ändert den fachlichen #124-Vertrag nicht.
