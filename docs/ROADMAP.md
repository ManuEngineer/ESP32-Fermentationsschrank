...
| 2 | Issue #90 – produktiver ESP-IDF-NVS-Adapter fuer `IStateStore` | `BLOCKED`; Phase A ist mit `NO_PROVEN_MANUFACTURER_CANDIDATE` abgeschlossen. Der exakte R5.4-Plan `docs/tasks/issue-90-esp-idf-nvs-adapter-plan.md @ c35ce0342898f0e19d3cce5e6a7eaa077f73bad6` bleibt unverändert und immutable. Der vollständige Host-`exhaustive`-Lauf bleibt am reproduzierten Callback 12 / `NotFound` `FAIL`; `f0c7d9b6...` und der offizielle Merge `0fa490...` sind bereits im gepinnten v6.0.2-Verhalten enthalten bzw. äquivalent und kein weiterer Lösungspfad. Phase B ist nicht freigegeben; reale Hardware bleibt `NOT_RUN`. | Ownerreview des Phase-A-Berichts; nur ein neuer exakter Herstellerstand kann ein neues Owner-Subgate begründen, andernfalls bleibt #90 `BLOCKED` |
...
- Issue #89 (WLAN-Onboarding-Evaluation) bleibt vor Beginn planpflichtig; Issue
  ...
  Phase A ist mit `NO_PROVEN_MANUFACTURER_CANDIDATE` abgeschlossen und der
  aktuelle Status ist `BLOCKED`. Phase B ist nicht freigegeben. Sein reproduzierbarer Callback-12-
