...
|---:|---|---|---|
| 1 | Issue #29 – ESP32-Bring-up, Partition, Ressourcen und sichere Ausgangszustaende | `ISSUE29=OPEN`; `PR129=OPEN_DRAFT`; `CURRENT_INTEGRATION_BASE=integration/r1-development @ 1fd8f6af53d1b3c23f3aa46c73c4fc3da7513d6d`; `PANIC_REPRODUCTION_SOURCE_SHA=c1f5fbb5f19ab8e7d2c25708fe79777d523217d4`; `PREVIOUS_HARDWARE_PASS=HISTORICAL`; `CURRENT_PROBE_REQUALIFICATION=CONTROL_BOOT_PANIC_REPRODUCED`; `PLAIN_BRINGUP_PANIC=CONFIRMED`; `CURRENT_BASE_CONTROL_BOOT=PANIC_REPRODUCED`; `CURRENT_BASELINE_PANIC_CONFIRMED=YES`; `CURRENT_BASE_CONTROL_SOURCE_SHA=7edda30de1d39d5a4945137146ab16da530c5dc6`; `CONTROL_BOOT_1=PANIC_REPRODUCED`; `CONTROL_BOOT_PROFILE=esp32_bringup`; `ROOT_CAUSE=UNRESOLVED`; `GPIO_SSOT=config/board_profiles/esp32_32e_quad_mosfet_r1.yaml`; `GPIO_MATRIX_STATUS=PLANNED_NOT_CONFIRMED`; `ELECTRICAL_VERIFICATION=PENDING`; `LEVEL_GATE=DEFERRED_PENDING_STABLE_REQUALIFICATION`; `EXHAUSTIVE_STATIC_GATE_ATTEMPT=SUPERSEDED_KISS`; `FULL_TRANSITIVE_STATIC_CALLGRAPH_CLOSURE=NOT_REQUIRED`; `STATIC_ANALYSIS_ROLE=DIAGNOSTIC_EVIDENCE_NOT_GLOBAL_UPPER_BOUND`; `HARDWARE_HWM_ROLE=PRIMARY_EMPIRICAL_STACK_EVIDENCE`; `CURRENT_MAX_KNOWN_STATIC_PATH_BYTES=72224`; `CURRENT_MAX_KNOWN_STATIC_PATH_IS_GLOBAL_UPPER_BOUND=NO`; `OLD_PROBE_TASK_STACK_BYTES=67584`; `COMPILED_runProbe_FRAME_BYTES=65712`; `OLD_PROBE_TASK_STACK_IS_BELOW_KNOWN_STATIC_PATH=YES`; `DIAGNOSTIC_PROBE_TASK_STACK_BYTES=98304`; `STACK96_IMPLEMENTATION_SOURCE_SHA=3d7b02260e18dd203cd609d97cc66fa96e435cdf`; `96_KIB_STACK_IMPLEMENTED=YES`; `96_KIB_BUILD_GATES=PASS`; `96_KIB_HARDWARE_RUN=ATTEMPTED_STOPPED_PROCEDURAL_INVALIDATION`; `BOOT_REQUALIFICATION=NOT_RUN`; `HWM_RESULT=INCOMPLETE_1_OF_3`; `IMPLEMENTATION=96_KIB_STACK_IMPLEMENTED`; `HARDWARE_RUN=96_KIB_BOOT1_ATTEMPT_STOPPED`; `LEVEL_MEASUREMENTS=NOT_RUN`; `ISSUE25_STARTED=NO`; `MERGE=NO`; `OWNER_PLAN_REVIEW=PASS`; `OWNER_CONTROL_BOOT_REVIEW_REQUIRED=YES`; `OWNER_96K_IMPLEMENTATION_REVIEW_REQUIRED=YES`; `OWNER_96K_HARDWARE_REVIEW_REQUIRED=YES`; `ANALYZER_REVERTED=YES`; `MAIN_CMAKE_ISSUE29_HEAP_INSTRUMENTATION_REVERTED=YES`; `DEVICE_PLATFORM_ISSUE29_INSTRUMENTATION_REVERTED=YES`; `CURRENT_BASE_CONTROL_BOOT_SPECIFIED=YES`; `HWM_ROOT_CAUSE_DISCRIMINATOR_SPECIFIED=YES`; `GPIO_SSOT_LEVEL_GATE_SYNC=YES`; `ROADMAP_ISSUE29_BASE_SYNC=YES`; `ROADMAP_ISSUE130=CLOSED_COMPLETED`; `ROADMAP_PR131=MERGED`; `NEW_PLAN_SHA=b7d80de7d6e23fd792c2bd48eaa27052a8c61201`. Details: Issue #29, `docs/ISSUE_29_MEASUREMENTS.md`, `docs/ISSUE_29_BUILD_REPORT.md`, `docs/tasks/issue-29-panic-requalification-correction-plan.md`; historische Plan-SHA `4a34967ac202196b7afceaebfe2b2429338d6d93` und bisherige KISS-SHA `42568610611ebffa6ace89e46f3fe3ea568e0e72` bleiben Provenienz. | Owner-Hardware-Review des prozedural invalidierten 96-KiB-Boot-1-Versuchs; erst nach freigegebener Capture-/Reset-Korrektur darf ein neuer 96-KiB-Bootlauf erfolgen. Pegelmessung erst nach `BOOT_REQUALIFICATION=3_OF_3_PASS` und Owner-Review. #29 bleibt von #90 getrennt; #25 bleibt zurückgestellt. |
...
Current-base-Kontrollpanic ist bestaetigt. Der 96-KiB-Kausaltest wurde im
Boot 1 wegen einer zusätzlichen Vorab-Resetsequenz prozedural gestoppt;
vor jedem weiteren Boot ist Owner-Review der Capture-/Reset-Prozedur
erforderlich.
