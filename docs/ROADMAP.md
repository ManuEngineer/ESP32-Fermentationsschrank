...
| 2 | Issue #90 – produktiver ESP-IDF-NVS-Adapter fuer `IStateStore` | `ISSUE90=OPEN`; `PR128=DRAFT`; `CURRENT_R1_DEVELOPMENT_BASE=integration/r1-development`; `PR123=MERGED`; kanonischer Korrekturplan: `docs/tasks/issue-90-slice7-host-utc-seam-correction-plan.md @ cdb19e52479db8a0db83270811c73910842c9393` (KISS, Owner-freigegeben); `IMPLEMENTATION=DIGITAL_PASS` (Commits `210d226`,`79add23`,`5bb7525`,`0dba45d`,`fb18fad`,`ecd9145`,`7b7522b`: gemeinsame `EspTimerTimeSource`, `SET_TRUSTED_UTC`-Harness-Befehl, `RecoveryEvaluation` in Runner-Oracle, esptool-Reset-/Versionsfixes, #29-Probe-Isolation vom Harness-Build, Reset-/Bootloader-Dokumentation korrigiert, UART-RX-Driver-Install-Fix); `ISSUE90_UART_RX_SMOKE=PASS` (real verifiziert: `ISSUE90_UART_RX_READY=PASS`, `ISSUE90_READY` mit korrekter Source-SHA, kein #29-Probe-Log, STATUS-Kommando beantwortet, `actuator_release=false`, 35s stabil ohne Panic/Watchdog/Reset); `HARDWARE_CAMPAIGN=REBASELINE_DONE_FRESH_PREPARE_REQUIRED` (Provenienz-Luecke aus `55e562b`/`0efd00c` behoben; `--phase rebaseline-production` auf Source-SHA `0efd00c` real auf Hardware ausgefuehrt: altes Manifest nur gelesen, nie geschrieben, `OLD_PARTITION_BACKUP_HASH=PASS`, `OLD_STATE_STORE_BACKUP_HASH=PASS`, Produktionsdaten byte-exakt aus dem alten Rohbackup wiederhergestellt und per Readback verifiziert (`PRODUCTION_DATA_BYTE_EXACT_RESTORED=PASS`, `PRODUCTION_PARTITION_TABLE_BYTE_EXACT_RESTORED=PASS`), neues Release-Image `0efd00c` frisch etabliert und per Readback verifiziert (`PRODUCTION_APPLICATION_REBASELINED=PASS`, SHA-256 `f68b24bf...`), Original-Applikations-Image ehrlich als unwiederbringlich verloren dokumentiert (`ORIGINAL_APPLICATION_BYTE_EXACT_RESTORE=NOT_POSSIBLE_ARTIFACT_LOST`), nach echtem Owner-Stromzyklus normaler Produktboot mit exakt passender Source-SHA verifiziert (`POST_REBASELINE_PRODUCT_BOOT=PASS`, `PRODUCTION_BASELINE_REESTABLISHED=PASS`), `rebaseline-record.json` Status `COMPLETE`); `REAL_POWER_CUTS=NOT_YET_AUTHORIZED`; historischer R5.9-Plan bleibt Grundlage: `docs/tasks/issue-90-clean-restart-plan-r5.9.md @ baf0b2ae04cd42afa75dfa00e21d900116b38bc8`. | Owner-Freigabe fuer frischen `prepare`-Zyklus mit neuem `--backup-dir`/`--manifest-output` (alte Pfade sind jetzt guard-gesperrt), danach Harness-Re-Flash + UART-Smoke-Wiederholung, danach Owner-Freigabe der sechs realen Power-Cuts. |
