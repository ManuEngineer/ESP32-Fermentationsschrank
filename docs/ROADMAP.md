...
| 2 | Issue #90 – produktiver ESP-IDF-NVS-Adapter fuer `IStateStore` | `SLICE_1_COMPLETED_PENDING_OWNER_REVIEW`; die exakte R5.7-Plan-SHA `docs/tasks/issue-90-product-recovery-replan-r5.7.md @ 4b4053bc229f279f5b30fc16ed60c765d1ee151e` ist ownerfreigegeben und bleibt immutable. Slice 1 synchronisiert die kanonischen ADR-, Persistenz-, Safety- und Quality-Verträge; die bestehenden öffentlichen Store-Statusfamilien sind ausreichend (`STATUS_TYPES_SUFFICIENT`). Der Callback-12-/`NotFound`-Befund bleibt als `BACKEND_POWER_CUT_CHARACTERIZATION` beziehungsweise `KNOWN_BACKEND_LIMITATION` sichtbar und ist nach dem unveränderten Release-1-Produktvertrag nicht alleiniger Produktblocker. R5.4 `docs/tasks/issue-90-esp-idf-nvs-adapter-plan.md @ c35ce0342898f0e19d3cce5e6a7eaa077f73bad6` bleibt die immutable ownerfreigegebene historische Blockerrevision. R5.5 `docs/tasks/issue-90-architecture-replan-r5.5.md @ 1dfa7eb390ededec4dc58efee7138d3fffdd39ff` und R5.6 `docs/tasks/issue-90-product-recovery-replan-r5.6.md @ a84ae061abc7199d8285cd142c139cd829cb66e9` bleiben nicht ownerfreigegeben und unangewendet. Der Phase-A.1-Bericht bleibt `docs/ISSUE_90_MANUFACTURER_ANALYSIS.md @ 440180f5b712a1eda427292207ebc7e9861cd284`. NVS bleibt Default; R5.7 begrenzt #90 auf beobachtbare Recoveryzustände und actor-free Boardverifikation und trennt Backendcharakterisierung vom Produktgate. Slice 2–6 bleiben `NOT_RUN` und benötigen jeweils ein eigenes ausdrückliches Owner-Subgate. | Ownerreview von Slice 1; danach nur ein jeweils separat freigegebener nächster Slice |
...
- Issue #89 (WLAN-Onboarding-Evaluation) bleibt vor Beginn planpflichtig; Issue
  ...
  bleibt Default. R5.7 ist mit der exakten SHA
  `4b4053bc229f279f5b30fc16ed60c765d1ee151e` ownerfreigegeben und bleibt
  immutable. Slice 1 ist mit `STATUS_TYPES_SUFFICIENT` abgeschlossen und wartet
  auf Ownerreview: `SLICE_1_COMPLETED_PENDING_OWNER_REVIEW`. Slice 2–6,
  Implementierung, Produkt-Recoverytests und reale Hardwareverifikation bleiben
  `NOT_RUN` und sind nicht freigegeben.
