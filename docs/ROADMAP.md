...
| 2 | Issue #90 – produktiver ESP-IDF-NVS-Adapter fuer `IStateStore` | `IMPLEMENTATION_BLOCKED_PENDING_R5_PLAN_APPROVAL`; Draft-PR #117 wartet auf Ownerfreigabe der exakten R5-Plan-SHA. Der vollständige Host-`exhaustive`-Lauf bleibt am reproduzierten Callback 12 / `NotFound` `FAIL`; die Empfehlung ist direkter NVS-Pfad mit exakt versionierter Espressif-Korrektur, andernfalls `BLOCKED`. Das #29-Pegel-Restgate blockiert die zulässigen aktorfreien NVS-Tests nicht; Power-Cut bleibt an ein ownerverifiziertes kontrollierbares Fixture gebunden. | Exakte R5-Plan-SHA freigeben; danach Herstellerkorrektur/Host-Exhaustive, erst dann reale #90-NVS-Nachweise |
