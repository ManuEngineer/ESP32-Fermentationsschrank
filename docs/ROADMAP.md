...
|---:|---|---|---|
| 1 | Issue #29 – ESP32-Bring-up, Partition, Ressourcen und sichere Ausgangszustaende | `ISSUE29=OPEN`; `PREVIOUS_HARDWARE_PASS=HISTORICAL` (PR #116, vor der aktuellen Gesamtbaseline); `CURRENT_R1_DEVELOPMENT_BASE=integration/r1-development @ c1f5fbb5f19ab8e7d2c25708fe79777d523217d4` (nach PR-#128-Merge); `LEGACY_PR116=CLOSED_SUPERSEDED`; `LEVEL_GATE=BLOCKED_OWNER_MEASUREMENT_PENDING`; `CURRENT_PROBE_REQUALIFICATION=FAILED_REPRODUCED_PANIC` (2026-08-30, realer Boot auf dem **normalen** `esp32_bringup`-Profil, ohne #90-Harness, actor-frei, unbelastet: `Guru Meditation Error (LoadProhibited)` deterministisch reproduziert, Backtrace gegen das exakt geflashte ELF dekodiert — `sampleResources` -> `runProbe` -> `probeTask` -> `heap_caps_get_largest_free_block` -> `heap_caps_get_info` -> `multi_heap_get_info_impl` -> `tlsf_walk_pool`/`block_is_last`; identischer Panic-Reboot-Zyklus 44x innerhalb des Beobachtungsfensters, kein Watchdog, kein Brownout, kein unerwarteter Reset — Reset ist der kontrollierte `SW_CPU_RESET` des Panic-Handlers; `actuator_release=false` in jedem Zyklus bis zum Absturz); `NEW_PROBE_PANIC=REPRODUCED_ON_PLAIN_BRINGUP`; `CODE_CHANGE=NO`; `OWNER_REVIEW_REQUIRED=YES`. Damit ist der vormals auf dem ungewollt gekoppelten `esp32_bringup_issue90`-Build beobachtete Panic nun auch auf dem normalen Profil bestaetigt, unabhaengig von #90. | Owner-Review des reproduzierten Panics und Freigabe eines vollstaendigen Korrekturplans, bevor `issue_29_bringup_probe.cpp` geaendert wird. Das offene Pegelgate (POWER_ON/RESET_EN/BOOTLOADER_IO0/ACTOR_FREE_NORMAL_BOOT) bleibt zusaetzlich unabhaengig offen und wird nicht als Ersatz fuer die Panic-Behebung verwendet. #29 bleibt von der #90-Power-Cut-Kampagne getrennt. |
