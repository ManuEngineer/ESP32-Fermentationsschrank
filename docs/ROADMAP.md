...
| 1 | Issue #27 – Web/API/Auth auf aktuellem `main` neu aufsetzen | `BASE_SHA=b871375f494701bed1834013cfeb789856983e3a`; `BRANCH=agent/issue-27-web-api-auth-main-restart`; `NEW_PR=DRAFT`; `PLAN_STATUS=INDEPENDENT_PLAN_FIX_VERIFICATION_PENDING`; `OWNER_PLAN_APPROVAL=REQUIRED_AFTER_FIX_VERIFICATION`; `IMPLEMENTATION_AUTHORIZATION=NO`; `OLD_PR167=SUPERSEDED_REFERENCE_ONLY`; `KDF_ALGORITHM=PBKDF2_HMAC_SHA256`; `KDF_WORK_FACTOR=10000`; `KDF_WORK_FACTOR_FALLBACK=NONE`; `ACTUATOR_RELEASE=NO` | Unabhaengige Plan-Fix-Verification auf der korrigierten exakten Plan-SHA; danach Owner-Planfreigabe; bis dahin keine Produktimplementation |
