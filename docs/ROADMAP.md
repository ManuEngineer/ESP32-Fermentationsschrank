...
| 2 | Issue #90 – produktiver ESP-IDF-NVS-Adapter fuer `IStateStore` | `ISSUE90=OPEN`; `PR128=DRAFT`; `CURRENT_R1_DEVELOPMENT_BASE=integration/r1-development`; `PR123=MERGED`; kanonischer Korrekturplan: `docs/tasks/issue-90-slice7-host-utc-seam-correction-plan.md @ cdb19e52479db8a0db83270811c73910842c9393` (KISS, Owner-freigegeben); `IMPLEMENTATION=DIGITAL_PASS` (Commits `210d226`, `79add23`: gemeinsame `EspTimerTimeSource`, `SET_TRUSTED_UTC`-Harness-Befehl, `RecoveryEvaluation` in Runner-Oracle ergaenzt); `RUNNER_SELFTEST=PASS`; `ISSUE90_PARTITION_GATE=PASS`; `ISSUE90_ISOLATION_GATE=PASS`; `ESP_IDF_BRINGUP_BUILD=PASS`; `ESP_IDF_RELEASE_BUILD=PASS`; `FULL_NATIVE_BUILD=PASS`; `FULL_NATIVE_TESTS=PASS_1081_CASES`; `ARCHITECTURE_GATES=PASS`; `GIT_DIFF_CHECK=PASS`; `HARDWARE_CAMPAIGN=NOT_RUN_PENDING_OWNER_IMPLEMENTATION_REVIEW`; historischer R5.9-Plan bleibt Grundlage: `docs/tasks/issue-90-clean-restart-plan-r5.9.md @ baf0b2ae04cd42afa75dfa00e21d900116b38bc8`. | Owner Implementation Review des Korrektur-Diffs; danach physische NVS-/Power-Cut-Kampagne (3x config, 3x run) mit manuellem ROM-Bootloader, Backup/Restore und Normalboot-Provenienz. |
