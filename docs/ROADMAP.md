...
| 4 | Issue #121 – Release-1 Device-/Application-Lifecycle- und Safety-Policy-Vereinfachung | `IMPLEMENTATION=STEP_6_IMPLEMENTED_PENDING_OWNER_RE_REVIEW`; `IMPLEMENTATION_STEP_6=IMPLEMENTED_PENDING_OWNER_RE_REVIEW`; Schritte 1–5 ownerreviewt PASS. Der freigegebene R1-Plan bleibt `docs/tasks/issue-121-lifecycle-safety-simplification-plan.md @ 68b81a16892a3a27c2eabdb8f571e34f1c107bbb`; `OWNER_PLAN_REVIEW=PASS`, `APPROVED_PLAN_SHA=68b81a16892a3a27c2eabdb8f571e34f1c107bbb`. Schritt 6 ist technisch abgeschlossen mit `STEP_6_DIAGNOSTIC_CORRECTION_SOURCE_SHA=5d8bc4d55d996c282fddd05a30ffde7184a12238` und demselben `STEP_6_EFFECTIVE_SOURCE_SHA`; die Korrektur umfasst ausschließlich `FaultCode::RunPersistenceUntrusted` → `FaultCode::None` im `publishStandby()`-Fehlerpfad. `OWNER_STEP_6_REVIEW=REQUIRED`, `PLAN_DEVIATION=NONE`, `STEP_6_PRE_PLAN_CORRECTION_EFFECTIVE_SOURCE_SHA=d016d7fc6c6d60a3e2145a386f793686f20a4200`, `NO_RUN_BOOT_TRANSITION_OWNER=PROCESS_STATE_MACHINE`, `FERMENTATION_APPLICATION_DIRECT_APPLY_PROCESS_TRANSITION=NO`, `ARCHITECTURE_CHECKER_CHANGE=NO`, `PROCESS_STATE_TOPOLOGY_CHANGE=NO`, `PROCESS_STATE_ENUM_CHANGE=NO`, `TRANSITION_REASON_CHANGE=NO`, `WIRE_FORMAT_CHANGE=NO`, `SCHEMA_MIGRATION_REQUIRED=NO`, `PRODUCTION_CODE_CHANGE=ONE_DIAGNOSTIC_LINE_ONLY`, `TEST_CODE_CHANGE=NO`, `PLAN_CODE_CHANGE=NO`, `IMPLEMENTATION_STEP_7=NOT_STARTED`, `HARDWARE_RUN=NO`, `PR120=OPEN_DRAFT_FROZEN`, `PR122=OPEN_DRAFT`, `ISSUE121=OPEN`, `MERGE=NO`, `SOURCE_OF_TRUTH_CONFLICT=NONE`. Gezielte Nachweise: `test_process_state_machine=36/36 PASS`, `test_smoke=12/12 PASS`, clang-format `PASS`, Architekturguard `PASS` via `python3`-Fallback (`python` ist in der Umgebung nicht vorhanden), `git diff --check=PASS`; keine vollständige Test-Suite, keine GitHub-Firmware-CI, kein Hardwarelauf und kein finales Produkt-Stackgate. | STOP – Owner Re-Review von Schritt 6 auf dem neuen effektiven Source-SHA; Schritt 7, Hardwarelauf und Merge bleiben ausgeschlossen. |
