...
| 2 | Issue #90 – produktiver ESP-IDF-NVS-Adapter fuer `IStateStore` | `R5_9_SLICE_5_COMPLETED_PENDING_OWNER_REVIEW`; PR #118 bleibt OPEN/Draft. Slice 5 fuehrt den einzigen generischen `NvsStateStore`-Adapter, den v6.0.2-Read-/Write-Vertrag, den actor-free Owning-Context-Lifecycle fuer `state_store`/`fermentation`, getrennte 4-MB-Produkt-/Testlayouts, einen transitiv pruefbaren ESP-IDF-Consumer und einen projektspezifischen Host-Harness ueber die offizielle v6.0.2-BDL-API ein. Das korrigierte NVS-Modell weist `4784` vollstaendige Slot-Keyspace-Entries, `1036` tatsaechliche Same-Key-Update-Transient-Entries, `0` weitere bewiesene Transient-Entries, `5820` technische Minimum-Entries/`47` Seiten/`0x2F000` Bytes sowie `4` GC-/Fragmentierungsreserve- und `64` Wear-Planungsreserve-Seiten aus; geplant sind `115` Seiten/`0x73000` und damit `141` Seiten Headroom in der unveraenderten `0x100000`-Partition. Der offizielle `nvs_get_stats()`-Crosscheck meldet `before_used=1`, `full_used=4784` und `updated_used=4784`. Clean-Linux-BDL und der separate oeffentliche Consumer-Build sind ohne Shim PASS (`14/14` Hosttests). Die vollstaendige BDL-Matrix ist `COMPLETE`, `exhaustive=true`; 14072 Writes und 503 Erases wurden beobachtet, GC-/Erase-Cuts sowie echte `rh0`-Write-Cuts vor/nach dem Zielereignis mit Frozen-Image-/Fresh-Process-Recovery sind PASS. Die beiden BDL-I/O-Failure-Faelle werden als tatsaechlich persistierter OLD-Head durch die Production-Bridge gefuehrt und sind PASS; der committed-NEW-Head ist ebenfalls PASS. Die Product-Bridge- und Cut-Artefakte tragen die exakte Source-SHA und sind validiert. Native: `987/987 PASS`; ESP-IDF `esp32_bringup`/`esp32_release`, Consumer und offizieller esp-clang-Static-Check: PASS. Callback 12 bleibt `FAIL_CALLBACK_12_NOT_FOUND`, `KNOWN_BACKEND_LIMITATION`, `REAL_NVS_ONLY`, `product_recovery_gate=NOT_RUN`. `SOURCE_OF_TRUTH_CONFLICT: NONE`; Oracle-Luecken: `NONE`; `PRODUCT_RECOVERY_MISMATCH: NONE`. | Ownerreview der vollstaendigen Slice-5-Evidenz; danach keine Slice-6-/Slice-7-Arbeit ohne separate Freigabe |
