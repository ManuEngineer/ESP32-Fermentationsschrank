...
| 4 | Issue #121 – Release-1 Device-/Application-Lifecycle- und Safety-Policy-Vereinfachung | `IMPLEMENTATION_STEP_8=BLOCKED`; freigegebener Plan `docs/tasks/issue-121-lifecycle-safety-simplification-plan.md @ 1568c8930aeb759468aeca1e27da93889875554a`; Schritte 1–7 PASS und ownerreviewt; `STEP_8_STATIC_ANALYSIS_SOURCE_SHA=6e3c722e36b28e00e9e7ffc84e44fdc8289a5f4`; reale Release-/Bring-up-Produkt-Callgraph-Evidenz zeigt `PRODUCT_BOOT_CUMULATIVE_STACK_PATH=12192 B` (Headroom `-8608 B`) und `PRODUCT_DISCARD_CUMULATIVE_STACK_PATH=3952 B` (Headroom `-368 B`) bei `CONFIGURED_RELEASE_MAIN_TASK_STACK=3584 B`; der statische Produkt-Stackgate ist FAIL, bevor Hardware autorisiert werden darf. `ESP_IDF_BRINGUP_BUILD=PASS` und `ESP_IDF_RELEASE_BUILD=PASS`; keine Produktionssemantik, kein Taskbudget-, Hardware- oder Merge-Schritt geändert. `PRODUCT_FRESH_START_CUMULATIVE_STACK_PATH=NOT_PRODUCT_REACHABLE`; `PRODUCT_RESUME_CUMULATIVE_STACK_PATH=NOT_PRODUCT_REACHABLE`; `HARDWARE_RUN=NO`; `PR120=OPEN_DRAFT_FROZEN`; `PR122=OPEN_DRAFT`; `ISSUE121=OPEN`; `MERGE=NO`; `PLAN_DEVIATION=NONE`; `SOURCE_OF_TRUTH_CONFLICT=NONE`. | STOP – Owner-Entscheid über den realen statischen Stackbefund; kein Produktionsumbau, keine Stack-Erhöhung, keine Hardwarefreigabe und kein Merge durch den Agenten. |
