...
| 2 | Issue #90 – produktiver ESP-IDF-NVS-Adapter fuer `IStateStore` | `IMPLEMENTATION_BLOCKED_PENDING_R5_2_PLAN_APPROVAL`; Draft-PR #117 wartet auf Ownerfreigabe der exakten R5.2-Plan-SHA. Der vollständige Host-`exhaustive`-Lauf bleibt am reproduzierten Callback 12 / `NotFound` `FAIL`; `f0c7d9b6...` ist bereits im gepinnten v6.0.2-Verhalten enthalten und kein offener Lösungs-Kandidat. Die Reihenfolge ist 1A Analyse/Evidenz, separates Owner-Subgate, 1B Herstellerpfadnachweis, danach R6-Korrekturen und finale Gates. OTA bleibt optionale Zukunft und ist kein #90-Reserve- oder Release-1-Gate. Das #29-Pegel-Restgate blockiert die zulässigen aktorfreien NVS-Tests nicht; Power-Cut bleibt an ein ownerverifiziertes kontrollierbares Fixture gebunden. | Exakte R5.2-Plan-SHA freigeben; danach 1A/Subgate/1B, R6-Korrekturen, finale Gates und erst dann reale #90-NVS-Nachweise |
