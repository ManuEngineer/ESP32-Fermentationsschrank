...
| 4 | Issue #121 – Release-1 Device-/Application-Lifecycle- und Safety-Policy-Vereinfachung | `PLAN_ONLY`; begrenzte Vereinfachung von Device/Application Lifecycle, Process Lifecycle, Actuation Permission, Boot Classification und Persistence Technical Result gemaess Ownerkorrekturen A-J zum Architektur-Audit. Kein Wire-/Schema-Bruch, keine neue Aktorfreigabe. Draft-PR auf Basis PR-#118-HEAD (`fd7e4e3`), nicht auf der eingefrorenen PR-#120-Implementation. `MATERIAL_ARCHITECTURE_DECISION_OPEN=YES` bis der vollstaendige Plan alle Ownerkorrekturen geschlossen enthaelt. | Vollstaendiger, eigenstaendiger Plan; danach Ownerreview der exakten Plan-SHA vor jeder Implementation. |
