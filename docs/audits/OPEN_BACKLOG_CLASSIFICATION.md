...
| **#30 – DS18B20-Busse und reale Sensoradapter** | zwingend | `BLOCKED_HARDWARE`; sekundaer `ADAPTER_EXISTING_LIBRARY` | Sensorport; Dallas+OneWire und Espressif-Kandidaten; #20/#21 laufen parallel | duennner, rollenunabhaengiger Adapter fuer Bus-/ROM-Konfiguration und Fehleruebersetzung | vollstaendig | Drei-Gate-DS18B20-Spike direkt nach minimaler Baseline und parallel zu #20–#24; danach Ownerwahl und kleiner Adapter-PR; Rollenprioritaet/Safety verbleiben im Kern; Phase 3–6 | **OD-03:** Kandidat anhand identischer Messung; Toolchain-Scheitern nicht als generelle Untauglichkeit werten |
...
5. Der minimale Baseline-Anteil von #29 und die aktorfreien Display-/Touch- und
   ...
   keine Vorbedingung.
6. #30 benoetigt vor der Produktivadapterwahl den dokumentierten
   Drei-Gate-Sensorspike. #31 benoetigt den gestuften Display-/Touchvergleich
   von Hardwareidentifikation bis Stufe-4-Ownerwahl; Reservekandidaten
   durchlaufen die Vollmatrix nicht automatisch. Die UI-/Sensorfachlogik bleibt
   davon getrennt, und LVGL wird erst nach Displaytreiber und Adaptervertrag
   bewertet.
