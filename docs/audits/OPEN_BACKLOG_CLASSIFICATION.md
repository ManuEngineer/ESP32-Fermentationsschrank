...
| **#19 – Journale, Aufbewahrung, Bereinigung, Backup und Import** | R1-Mindestumfang ja, heutiger Umfang sehr breit | `CUSTOM_APPLICATION`; sekundaer `ADOPT_LIBRARY` | `IEventJournal`, typisierte Konfiguration und binaeres Wireformat; ArduinoJson `7.4.3` als bevorzugter `SPIKE_REQUIRED`-Kandidat nur fuer externe Vertraege | Journal-/Retentionpolicy, begrenzte Historie, redigierter Export, portables secret-freies Backup, typisierte Importvorschau und Bestaetigung; keine Aktivierung aus dem Parser | reale NVS-/Flash-/Heapbudgets sowie JSON-Grenz-/Fuzz-/Ressourcennachweis | in binaeres Journal/Retention, begrenzten Export und JSON-Backup/Import teilen; interne Kontrollpunkte bleiben binaer, grosse Daten werden paginiert/gestreamt; erst nach #17 und dem neu geschnittenen #56-R1-Kern; Phase 2/3/7 | **OD-07:** Mindestumfang und PR-Schnitt bestaetigen; JSON-Richtung beschlossen, endgueltige Codec-Uebernahme nach Spike |
