...
Live-Stand: 2026-07-27 (Original-Audit). Erfasst sind alle offenen
Implementierungs- und Tracking-Issues #16–#37 sowie #56 und #57. Die offenen
Epiccontainer #3–#8 und das Audit-Issue #62 sind keine Implementierungsissues
und werden deshalb nicht als eigene Arbeitsbausteine klassifiziert. Kein
Issue wurde durch den Original-Audit veraendert; die am Dokumentende
dokumentierte Espressif-first-Synchronisierung vom 2026-08-05 bildet davon
eine ausdruecklich ownerfreigegebene Ausnahme fuer #27, #30, #31, #89 und
#90.
...
|---|---|---|---|---|---|---|---|
| **#16 – Konfigurationsebenen, Validierung und atomare Revisionen**: Tracking fuer #54–#57 | zwingend, aber nicht selbst implementierbar | `CUSTOM_APPLICATION`; sekundaer `ADAPTER_EXISTING_LIBRARY` | #54/#55 gemergt: Storeport, Wireformat, Slots, Dokumente; ESP-IDF NVS gemaess ADR-016 | Tracking und Abnahme des entschiedenen Variante-B-Kerns; spaetere Variante-A-Arbeit getrennt halten | NVS-Budget und Power-Cut real, Logik nativ | nicht unveraendert fortsetzen; nach dem Audit in separatem ownerfreigegebenem Planungs-/ADR-Schritt auf R1-Kern und spaetere additive Arbeit neu schneiden; Phase 1/2 | OD-01 ist entschieden; Audit aendert Issue oder ADR nicht |
| **#17 – Laufpersistenz und Kontrollpunkte** | abgeschlossen durch PR #84 (Merge-Commit `8d65b50326c4419dc45bbc024615c0a1c592e1aa`) | `CUSTOM_APPLICATION` (erfuellt) | Laufschnappschuss, Laufrevisionen, Zustandsmaschine, `IStateStore`, Envelope/Slots, Kontrollpunktmodell, Codec, Rueckfall und Korruptionserkennung aus PR #84 implementiert; Schema 1 sowie Active-/Fallback-/Head-Vertrag unveraendert | kein weiterer Scope in #17 | produktives ESP-IDF-NVS-Backend und reale NVS-/Cut-Verifikation separat in #90 | #17 ist abgeschlossen und die schmale Grundlage fuer #18/#19/#24; keine neue #17-Implementierung vorsehen | keine offene Entscheidung fuer #17; NVS-Adapterumsetzung bleibt #90 |
| **#18 – Wiederanlauf und temperaturgewichteter Fortschritt** | zwingend | `CUSTOM_APPLICATION`; sekundaer `CUSTOM_SAFETY_CORE` | virtuelle Zeit, Zustandsmaschine, Laufmodelle; #17 abgeschlossen (PR #84), #20 fehlt | phasenbezogener Recoveryentscheid, Unsicherheitsintervall und Fortschrittskorrektur | echte Unterbrechungsdaten/Temperaturverlaeufe spaeter | baut auf dem abgeschlossenen #17-Fundament auf, nach #20, vor Endintegration; Phase 2 | keine Bibliotheksentscheidung; Scope als mehrere kleine PRs schneiden |
...
| **#27 – Web-API, Weboberflaeche, Anmeldung und Konflikte** | R1 als sekundaere lokale Bedienoberflaeche; Issue im Espressif-first-Sync gezielt um HTTP-Kandidaten und die Trennung zu #89 ergaenzt, fachlicher Fuenferschnitt unveraendert | `CUSTOM_APPLICATION`; technische Kandidaten spikepflichtig | Fachkommandos, #25, `INetworkStatus`; ESP-IDF `esp_http_server`/ArduinoJson zuerst, Async ergebnisoffener Evaluationskandidat; Onboarding separat | A Transport/API; B Status/Polling/Chart; C Mutationen/Konflikte; D Assets; E passwortgeschuetzte oder anonyme Session/Auth; normale Weboberflaeche auch im geschuetzten Ersatz-WLAN | WLAN-/Client-/Jitter-/Ressourcen-/Auth-/CSRF-/Credentialtests; Ersatz-WLAN-Lifecycle und offene Requests | produktive Mutationen erst nach erfolgreichem technischen Auth-/CSRF-/Credential-/Ressourcen-, Webserver- und JSON-Nachweis; passwortlos weiterhin anonyme Session, CSRF und Warnung; Ersatz-WLAN koordiniert #27, Onboarding bleibt OD-06 | OD-07/OD-09/ADR-017 entschieden; technische Produktfreigabe bleibt offen |
...
3. #17 ist durch PR #84 abgeschlossen und haengt nicht pauschal vom gesamten
   Tracking-Issue #16 oder von Variante-A-Funktionen ab; die tatsaechlich
   benoetigten gemergten Persistenzbausteine und schmalen R1-Vertraege waren
   ausreichend. Der produktive ESP-IDF-NVS-Adapter bleibt separat #90.
...
9. Das WLAN-Onboarding bleibt als OD-06-Arbeitsbereich getrennt von #27
   ...
   veraendert den bisher funktionierenden Stand nicht.
...
Der urspruengliche Audit veraenderte die Live-Issues und ADRs nicht. Die
Espressif-first-Synchronisierung (2026-08-05) bildet eine ausdruecklich
...
und #90 wurden erstellt; keine ADR wurde geaendert oder erstellt. Der
verbindlich notwendige Neuschnitt und alle spaeteren Issues fuer Pending oder
reale Secret-Domaenen
