...
| **#27 – Web-API, Weboberflaeche, Anmeldung und Konflikte** | R1 als sekundaere lokale Bedienoberflaeche; sicherer Betrieb ohne WLAN/Browser | `CUSTOM_APPLICATION`; sekundaer `CONFIGURE_FRAMEWORK`, konditional `ADOPT_LIBRARY` | Kommandos, Konfigurationsmodelle, `INetworkStatus`; Arduino-ESP32 `WebServer` als R1-Baseline, ESPAsync nur Rueckfall; ArduinoJson/WiFiManager Kandidaten | serverunabhaengige begrenzte DTO-/Endpunktlogik, API, Webviews, Authpolicy, Sessions, CSRF, Konflikte, Redaction und erste reale Authentication-/Connectivity-Nachweise; kleiner konkreter Serveradapter ohne Servertypen im Fachkern | WLAN-/Browser-/Abbruch-/Jitter-/Last- und Ressourcentest | zuerst Frameworkserver-Prototyp; Transportintegration, API, Webassets, Auth und Onboarding getrennt schneiden; Async nur bei konkretem Scheitern oder klarem Vorteil im identischen Prototyp; spaeterer Serverwechsel ueber Composition Root/Integrationsgrenze, keine allgemeine Transportplattform; reale Secret-Domaene erst mit diesem Konsumenten und nach OD-09; Phase 7 | **OD-04 entschieden:** Frameworkserver-Baseline, Abweichung nur nach Nachweis; **OD-06/OD-09:** Onboarding und Authvertrag |
