...
| Arduino-ESP32 WebServer | [Espressif Arduino-ESP32](https://github.com/espressif/arduino-esp32) | Projekt verwendet `2.0.17`, Framework-SHA `dcc1105b`; `FIRST_EVALUATION_CANDIDATE`, `SPIKE_REQUIRED`, `FINAL_SELECTION_PENDING` | Bestandteil des fixierten Frameworkpakets; LGPL-2.1 und eingebettete Drittkomponentenhinweise des konkreten Pakets gelten | interne begrenzte Baselineevaluation fuer lokalen HTTP-Transport; noch keine Produktivuebernahme und keine neue Drittbibliothek | konkret verwendete Frameworkdateien sowie Lizenz-/Notice-Abdeckung des Firmwareartefakts im umsetzenden PR erfassen | `FRAMEWORK_PRESENT` |
...
| ESPAsyncWebServer | [ESP32Async/ESPAsyncWebServer](https://github.com/ESP32Async/ESPAsyncWebServer) | `3.12.0`, `a008cccf`; aktiv 2026-07; `CONDITIONAL_FALLBACK`, `EVALUATE_LATER` | Top-Level `LICENSE`: LGPL-3.0; asynchrone TCP- und optionale JSON-Abhaengigkeiten separat | nur konditionale Rueckfallevaluation bei nachgewiesenem Problem des ersten `WebServer`-Prototyps; nicht ausgewaehlt | vor einem identischen Vergleich und jeder Einbindung dynamische/kompilierte Verlinkung, Source-Angebot/Notices und alle Abhaengigkeiten fuer den konkreten Distributionsweg pruefen | `PUBLICATION_REVIEW_REQUIRED` |
| WiFiManager | [tzapu/WiFiManager](https://github.com/tzapu/WiFiManager) | Tag `v2.0.17`, `d82d0a1b`; Upstream-HEAD `4131fe61`, aktiv 2026-02; bevorzugter `FIRST_EVALUATION_CANDIDATE`, `SPIKE_REQUIRED`, `FINAL_SELECTION_PENDING` | Top-Level `LICENSE`: MIT; eingebettete Webassets und transitive Abhaengigkeiten im konkreten Paket aufloesen | interne Evaluation und begrenzter, von #27 getrennter R1-Onboardingspike auf dem fixierten Tag; noch keine endgueltige Uebernahme, kein direkter Bibliotheksfork geplant | vor Distribution exakte Version, LICENSE, verwendete/veraenderte Dateien, Assets, Notices, Abhaengigkeiten und publizierten Umfang nachweisen; Anpassungen getrennt dokumentieren | `SPIKE_REQUIRED` |
| mbedTLS-/ESP32-Authentisierungspfad | [Espressif Arduino-ESP32](https://github.com/espressif/arduino-esp32) und darin verwendete mbedTLS-/ESP-IDF-Komponenten | fixierte Arduino-ESP32-Toolchain `2.0.17`; konkrete PBKDF2-HMAC-SHA-256-, Zufalls- und Vergleichsfunktionen noch im Spike zu bestimmen | Framework LGPL-2.1 sowie Lizenzen/Notices der konkret verwendeten eingebetteten mbedTLS-/ESP-IDF-Dateien pruefen | nur interne Evaluation fuer KDF-Testvektoren, kryptografischen Zufall und konstanten Vergleich; keine neue Bibliothek, keine endgueltige Produktionswahl und kein festgelegter Work Factor | konkret verwendete Funktionen/Dateien, Versionen, Buildkonfiguration, Lizenz-/Noticepflichten und publizierten Umfang vor einer Uebernahme dokumentieren | `SPIKE_REQUIRED` |
| ESP32-NVS-/Flashverschluesselung | [Espressif ESP-IDF](https://github.com/espressif/esp-idf) als Toolchain-/Herstellerreferenz | nicht aktiviert oder projektbezogen getestet; `EVALUATE_BEFORE_RELEASE` | Apache-2.0 plus eingebettete Drittbestandteile; konkreter Toolchainpfad und Produktionsprozess offen | separater interner Security-Spike zu Partitionierung, Provisionierung, Schluesselverwaltung, Recovery und Update; keine Aktivierung oder Schutzbehauptung im Audit | verwendete Komponenten, Produktions-/Flashprozess, Schluesselmodell, Notices und dokumentierte physische Schutzgrenze erst nach Ownerentscheid festhalten | `AUDIT_ONLY` |
