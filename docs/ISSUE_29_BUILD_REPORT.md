...
`build/issue29_requalification/corrected_96k_boot{1,2,3}_raw_uart.log` mit
...
`idf.py flash`-Versuch wurde vor dem Flash abgebrochen, weil ESP-IDF die
Flash-Aktion automatisch um `all`/Build erweiterte; die freigegebenen ELF-/BIN-
Dateien blieben danach hashgleich und wurden anschliessend ausschließlich per
direktem, buildfreiem `esptool write-flash` verwendet.

Der UART-Capture enthält zwei `rst:`-Sequenzen: eine unvollständige
Vorabsequenz während der Capture-/RTS-Initialisierung und danach die bewusst
ausgelöste vollständige Run-Sequenz. Dadurch ist die geforderte eindeutige
kanonische Bootzählung prozedural invalidiert. Boot 2 und Boot 3 wurden nicht
ausgeführt.
...
Der vollständige beobachtete Lauf erreichte den Probe-Erfolg und blieb danach
über 40 Sekunden stabil; er liefert jedoch wegen der zusätzlichen
Vorabsequenz keinen gültigen 3-Boot-Nachweis. Die HWM-Werte sind deshalb nur
