...
SUPERSEDES_PLAN_SHA=ea67786f99eeaf206e1b4b70eb3ff601d0e10941
PLAN_ONLY=YES
THIS_ROUND=PLAN_CORRECTION_ONLY
CORRECTION_IMPLEMENTATION_ALLOWED=NO_PENDING_FINAL_OWNER_PLAN_REVIEW
...
F5=OWNER_DECISION_OPTION_A_RECORDED
...
Test-, Build-, Dependency-, Boardprofil-, GPIO-, RTC-Adapter- oder
Hardwarechange. Insbesondere entstehen kein DS3231M-Support, kein DS3231SN-
Rework, kein Hardware-Issue, kein Hardware-Branch und kein Hardware-PR.
...
| F5 | `CONFIRMED`; `OWNER_DECISION_RECORDED` | offline Normalbetrieb in `REQUIREMENTS.md`/`PRODUCT_VISION.md`; RTC-less/NTP-only plus trusted-UTC Startgate in `ARCHITECTURE.md`/`HARDWARE.md`; Ownerentscheidung dieses Auftrags | ausschließlich die entschiedene Option-A-Produktpolicy dokumentieren |
...
FINDING_F5_REQUIRED_CHANGE=record Option A policy in current documentation only
...
## 4. F5: entschiedene Produktpolicy und weiter offener Hardwarenachweis

```text
SLICE_2=F5_OWNER_SELECTED_POLICY_DOCUMENT_ALIGNMENT
F5_OWNER_DECISION_REQUIRED=YES
F5_OWNER_DECISION_OPEN=NO
...
OWNER_DECISION_GATE=SATISFIED_BY_OPTION_A
CORRECTION_IMPLEMENTATION_ALLOWED=NO_PENDING_FINAL_OWNER_PLAN_REVIEW
...
RTC_PHYSICAL_VARIANT_CONFIRMED=NO
...
DO_NOT_ASSUME_DS3231SN=YES
DO_NOT_ASSUME_DS3231M=YES
MULTI_RTC_ARCHITECTURE=NO
GENERIC_RTC_PROVIDER_LAYER=NO
PREEMPTIVE_DS3231M_SUPPORT=NO
AUTO_CREATE_RTC_ISSUE=NO
AUTO_CREATE_RTC_BRANCH=NO
AUTO_CREATE_RTC_PR=NO
```

Option A ist verbindlich entschieden. Option B ist als verworfene
Entscheidungsalternative nicht Teil des ausführbaren Zielvertrags und wird
nicht implementiert.
...
Die generische Zeitplattform bleibt RTC-optional und NTP-only-fähig. Nur das
konkrete Fermenter-R1-Produkt verlangt eine lokale RTC, damit ein neuer
produktiver Lauf ohne WLAN/Internet starten kann; ohne trusted UTC wird kein
neuer Lauf zugelassen.

Die bestellte reale Hardware ist nur als DS3231-Familie bekannt. Weder
`DS3231SN` noch `DS3231M` darf vor der physischen Bestätigung angenommen
werden. Diese Planrevision führt deshalb weder Multi-RTC-Unterstützung noch
einen generischen RTC-Provider oder vorsorglichen DS3231M-Support ein.

Der historische/digitale Vertrag von Issue #126 bleibt unverändert:

```text
ISSUE126_HISTORICAL_CONTRACT_PRESERVED=YES
ISSUE126_DS3231SN_IMPLEMENTATION_REWRITTEN=NO
ISSUE126_REOPEN=NO
```

Erst nach Hardwareeingang wird außerhalb dieses Plans die tatsächliche
Variante festgestellt und gegen den bestehenden Adapter geprüft:

```text
ACTUAL_RTC_VARIANT=<DS3231SN|DS3231M|OTHER>

IF_DS3231SN:
  EXISTING_ISSUE126_ADAPTER_EXPECTED_MATCH=YES
  ONLY_REAL_FUNCTIONAL_VERIFICATION_REQUIRED=YES

IF_DS3231M:
  EXISTING_ADAPTER_COMPATIBILITY_REVIEW_REQUIRED=YES
  MINIMAL_REQUIRED_ADAPTATION_ONLY=YES
  PREEMPTIVE_MULTI_VARIANT_SUPPORT=NO
```

Für diesen realen Nachweis existiert derzeit kein passender offener
Hardware-Owner: #30, #31 und #33 betreffen Sensor-, Display-/Touch- bzw.
Peltierhardware; #126 ist geschlossen. Daher bleibt
`RTC_HARDWARE_OWNER=UNASSIGNED`. #136 dokumentiert nur Produktpolicy und
offenen Nachweis; es eröffnet weder einen zweiten RTC-Workflow noch ein neues
Tracking-Artefakt. Der Owner entscheidet später über einen passenden Scope.
...
2. Die in Abschnitt 4 entschiedene Option A ausschließlich in aktuellen
   Produkt-/Statusdokumenten ausrichten. Die Variantenbestätigung und reale
   Hardware-/Kompatibilitätsprüfung bleiben außerhalb dieses Scopes. Vor Slice
   2 ist die erneute Freigabe der exakten Plan-SHA erforderlich.
...
historic rewrite; Option A is documented, the real RTC proof stays
`UNASSIGNED` pending later Owner tracking, and the actual DS3231 variant stays
unassumed; exactly three F6 boundaries follow their existing error/null
contract and do not claim false success; and no GPIO, pull, net, design,
schema, actor-release or hardware state changes.
...
GPIO_CHANGE=NO
RTC_ADAPTER_CHANGE=NO
DS3231M_SUPPORT_IMPLEMENTATION=NO
DS3231SN_REWORK=NO
NEW_HARDWARE_ISSUE=NO
NEW_HARDWARE_BRANCH=NO
NEW_HARDWARE_PR=NO
...
MERGE=NO
ISSUE25_STARTED=NO
OWNER_FINAL_PLAN_REVIEW_REQUIRED=YES
```
