...
Diese Regel gilt fuer Claude-Code-Arbeiten an einem bestehenden Branch oder Pull
Request. Sie wird ueber `CLAUDE.md` in jede neue Claude-Code-Session geladen.

## Zweck

Ein Kontextreset mit `/clear` darf keinen relevanten Arbeitsstand verlieren. Der
Agent muss deshalb am Ende jedes Auftrags oder abgeschlossenen Arbeitsslices
automatisch eine vollstaendige, aktuelle Uebergabe als Kommentar im zugehoerigen
Pull Request veroeffentlichen. Der Owner muss diese Uebergabe nicht jeweils
separat anfordern.

`/clear` selbst wird ausschliesslich durch den Owner ausgefuehrt. Der Agent darf
keinen Kontextreset vortaeuschen, keine alte Session fortsetzen und den Owner
nicht auffordern, vor der veroeffentlichten Uebergabe zu loeschen.
...
7. danach anhalten und dem Owner mitteilen, dass `/clear` nun gefahrlos moeglich
   ist.
...
1. `AGENTS.md` und `CLAUDE.md` lesen.
