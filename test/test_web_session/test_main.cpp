...
void test_service_lease_status_is_read_only_and_uses_its_policy() {
    ...
    TEST_ASSERT_TRUE(initial.remainingMillis.has_value());
    TEST_ASSERT_EQUAL_UINT64(60U, *initial.remainingMillis);
    const auto later = sessions.serviceLeaseStatus(*created.handle, 99U);
    TEST_ASSERT_TRUE(later.active);
    TEST_ASSERT_TRUE(later.remainingMillis.has_value());
    TEST_ASSERT_EQUAL_UINT64(1U, *later.remainingMillis);
    TEST_ASSERT_FALSE(
