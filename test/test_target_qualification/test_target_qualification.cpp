...
void test_first_in_band_sample_starts_empty_episode() {
    ...
    const auto result = evaluator.evaluate(
        input(QualificationPhase::Target, 100U, 20.0));
...
void test_band_boundary_is_inclusive() {
    TargetQualificationEvaluator evaluator;
    const auto result = evaluator.evaluate(
        input(QualificationPhase::Target, 100U, 20.5));
    TEST_ASSERT_TRUE(result.progress == QualificationProgress::InBand);
}
...
void test_unavailable_and_invalid_interrupt_episode_differently() {
    ...
    TEST_ASSERT_TRUE(unavailable.progress == QualificationProgress::Unavailable);
...
void test_outside_starts_grace_and_direct_return_does_not_credit_time() {
    ...
    static_cast<void>(evaluator.evaluate(
        input(QualificationPhase::Target, 100U, 20.0)));
    const auto grace = evaluator.evaluate(
    ...
    const auto returned = evaluator.evaluate(
        input(QualificationPhase::Target, 400U, 20.0));
...
void test_grace_equality_expires_old_episode_before_return() {
    ...
    static_cast<void>(evaluator.evaluate(
        input(QualificationPhase::Target, 200U, 21.0)));
    const auto expired = evaluator.evaluate(
        input(QualificationPhase::Target, 700U, 20.0));
...
void test_gap_and_retrograde_time_reset_as_invalid() {
    ...
    static_cast<void>(evaluator.evaluate(
        input(QualificationPhase::Target, 100U, 20.0)));
    const auto retrograde = evaluator.evaluate(
        input(QualificationPhase::Target, 99U, 20.0));
    TEST_ASSERT_TRUE(retrograde.progress == QualificationProgress::Invalid);

    static_cast<void>(evaluator.evaluate(
        input(QualificationPhase::Target, 200U, 20.0)));
    const auto gap = evaluator.evaluate(
        input(QualificationPhase::Target, 2'201U, 20.0));
