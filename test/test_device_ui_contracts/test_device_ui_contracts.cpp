...
TextKey key(const char* value) {
    return {TextNamespace{"platform"}, value};
}
...
void test_text_resolver_uses_active_then_english_then_visible_key() {
    ...
    const auto german = device_platform::resolveText(
        packs, LocaleId{"de"}, key("save"));
    TEST_ASSERT_TRUE(german.source == TextLookupSource::ActiveLocale);
    TEST_ASSERT_EQUAL_STRING("Speichern", german.value.c_str());

    const auto englishFallback = device_platform::resolveText(
        packs, LocaleId{"es"}, key("cancel"));
    ...
    const auto visibleKey = device_platform::resolveText(
        packs, LocaleId{"es"}, key("missing"));
...
void test_theme_falls_closed_to_complete_included_default() {
    ...
    const auto fallback = device_platform::resolveTheme(
        themes, catalog, ThemeId{"other"});
...
void test_platform_sections_precede_isolated_application_sections() {
    const device_platform::StaticUiExtensionCatalog catalog{
        {{UiSectionOwner::Application, key("app-status"),
         ...
         {UiSectionOwner::Platform, key("network"),
