...
void test_state_store_round_trips_a_value() {
    ...
    const auto value = store.read("example_key");
    TEST_ASSERT_TRUE(value.has_value());
    TEST_ASSERT_EQUAL_STRING("0.5", value->c_str());
}

void test_state_store_missing_key_returns_nullopt() {
    const device_platform::MockStateStore store;

    TEST_ASSERT_FALSE(store.read("unknown_key").has_value());
}
...
void test_state_store_write_failure_is_injectable_and_preserves_old_value() {
    ...
    TEST_ASSERT_TRUE(value.has_value());
    TEST_ASSERT_EQUAL_STRING("first", value->c_str());
}
...
void test_state_store_read_failure_is_injectable() {
    ...
    TEST_ASSERT_FALSE(store.read("key").has_value());

    store.injectReadFailure(false);
    TEST_ASSERT_TRUE(store.read("key").has_value());
}
...
void test_event_journal_records_entries_in_order() {
    ...
    journal.record(100, "boot");
    journal.record(200, "standby");
...
int main() {
    ...
    RUN_TEST(test_state_store_missing_key_returns_nullopt);
    RUN_TEST(
