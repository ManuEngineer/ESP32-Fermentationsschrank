...
void test_first_valid_sample_stays_stale_but_updates_raw_evidence() {
    ...
    const auto disposition = pipeline.ingest(okReading(std::nullopt, 0U, 20.0), 0U);
...
void test_single_crc_fault_drops_to_stale_not_immediately_failed() {
    ...
    (void)pipeline.ingest(faultReading(std::nullopt, 4000U, TemperatureSampleStatus::CrcFault),
                    4000U);
...
void test_consecutive_crc_faults_beyond_ceiling_reach_failed() {
    ...
    (void)pipeline.ingest(faultReading(std::nullopt, 1000U, TemperatureSampleStatus::CrcFault), 1000U);
    (void)pipeline.ingest(faultReading(std::nullopt, 2000U, TemperatureSampleStatus::CrcFault), 2000U);
    (void)pipeline.ingest(faultReading(std::nullopt, 3000U, TemperatureSampleStatus::CrcFault), 3000U);
    TEST_ASSERT_TRUE(pipeline.snapshot(3000U).quality != SensorQuality::Failed);

    (void)pipeline.ingest(faultReading(std::nullopt, 4000U, TemperatureSampleStatus::CrcFault), 4000U);
...
void test_bus_fault_is_recorded_with_its_own_reason() {
    ...
    (void)pipeline.ingest(faultReading(std::nullopt, 4000U, TemperatureSampleStatus::BusFault),
                    4000U);
...
void test_missing_sample_is_recorded_with_its_own_reason() {
    ...
    (void)pipeline.ingest(
        faultReading(std::nullopt, 4000U, TemperatureSampleStatus::MissingSample),
        4000U);
...
void test_nonfinite_celsius_is_nonfinite_not_out_of_range() {
    ...
    (void)pipeline.ingest(
        okReading(std::nullopt, 4000U, std::numeric_limits<double>::quiet_NaN()),
        4000U);
...
void test_future_timestamp_is_rejected() {
    ...
    TEST_ASSERT_EQUAL_UINT16(0U, pipeline.snapshot(2000U).consecutiveInvalidCount);
}
...
void test_exact_duplicate_is_ignored() {
    ...
    TEST_ASSERT_EQUAL_UINT16(0U, pipeline.snapshot(1000U).consecutiveInvalidCount);
}
...
void test_same_timestamp_different_value_is_conflict() {
    ...
    TEST_ASSERT_TRUE(disposition == SampleDisposition::RejectedTimestampConflict);
}
...
void test_same_timestamp_same_value_different_identity_is_conflict_not_duplicate() {
    ...
    const auto disposition = pipeline.ingest(okReading(identityB, 1000U, 20.0), 1000U);
...
void test_same_timestamp_both_nan_is_duplicate() {
    ...
    const auto disposition = pipeline.ingest(okReading(std::nullopt, 1000U, nan), 1000U);
...
void test_same_timestamp_nan_versus_finite_is_conflict() {
    SensorQualityPipeline pipeline(makeTestConfig());
    (void)pipeline.ingest(
        okReading(std::nullopt, 1000U, std::numeric_limits<double>::quiet_NaN()),
        1000U);

    const auto disposition = pipeline.ingest(okReading(std::nullopt, 1000U, 5.0), 1000U);
...
void test_very_first_sample_is_accepted() {
    ...
    const auto disposition = pipeline.ingest(okReading(std::nullopt, 500U, 20.0), 900U);
...
void test_very_first_sample_from_the_future_is_rejected() {
    ...
    const auto disposition = pipeline.ingest(okReading(std::nullopt, 5000U, 20.0), 1000U);
...
void test_resumption_after_gap_is_not_evaluated_as_a_jump() {
    ...
    TEST_ASSERT_TRUE(disposition == SampleDisposition::Accepted);
    TEST_ASSERT_TRUE(pipeline.snapshot(oneHourMs).lastFaultReason ==
...
void test_value_below_absolute_minimum_is_out_of_range() {
    ...
    (void)pipeline.ingest(
        okReading(std::nullopt, 0U, device_platform::sensor_limits::kAbsoluteMinCelsius - 1.0),
        0U);
...
void test_value_above_absolute_maximum_is_out_of_range() {
    ...
    (void)pipeline.ingest(
        okReading(std::nullopt, 0U, device_platform::sensor_limits::kAbsoluteMaxCelsius + 1.0),
        0U);
...
void test_same_absolute_delta_is_judged_differently_over_short_versus_long_interval() {
    ...
    (void)shortInterval.ingest(okReading(std::nullopt, 200U, 23.0), 200U);  // 15 C/s
    ...
    (void)longInterval.ingest(okReading(std::nullopt, 1000U, 23.0), 1000U);  // 3 C/s
...
void test_rate_exceeding_sample_does_not_stop_permanently() {
    SensorQualityPipeline pipeline(makeTestConfig());
    (void)pipeline.ingest(okReading(std::nullopt, 0U, 20.0), 0U);
    (void)pipeline.ingest(okReading(std::nullopt, 200U, 23.0), 200U);  // exceeds rate
...
void test_renewed_invalid_sample_during_recovery_resets_progress() {
    ...
    (void)pipeline.ingest(okReading(std::nullopt, 5000U, 20.0), 5000U);
    (void)pipeline.ingest(faultReading(std::nullopt, 6000U, TemperatureSampleStatus::CrcFault), 6000U);

    const auto snapshot = pipeline.snapshot(6000U);
    TEST_ASSERT_EQUAL_UINT16(0U, snapshot.recoveryProgressCount);
    // Stale, NICHT Failed: die zwischenzeitliche gueltige Probe hat
    // consecutiveInvalidCount_ bereits auf 0 zurueckgesetzt, ein einzelner
    // erneuter Fehler bringt ihn nur auf 1 (< kMaxConsecutiveInvalid). Ein
    // Zurueckspringen auf Failed wuerde eine zweite, separat gespeicherte
    // Qualitaetswahrheit erfordern, die Abschnitt 9a bewusst ausschliesst -
    // quality ist ausschliesslich aus den rohen Zaehlgroessen abgeleitet.
    TEST_ASSERT_TRUE(snapshot.quality == SensorQuality::Stale);
...
void test_identity_change_between_two_known_identities_is_identity_mismatch() {
    ...
    const auto snapshot = pipeline.snapshot(1000U);
    TEST_ASSERT_TRUE(snapshot.lastFaultReason == SensorFaultReason::IdentityMismatch);
    TEST_ASSERT_TRUE(snapshot.quality == SensorQuality::Stale);
    TEST_ASSERT_EQUAL_UINT16(1U, snapshot.consecutiveInvalidCount);
}
...
void test_two_independent_pipelines_do_not_influence_each_other() {
    ...
    TEST_ASSERT_TRUE(air.snapshot(0U).lastFaultReason == SensorFaultReason::None);
...
void test_consecutive_invalid_count_saturates_instead_of_wrapping() {
    ...
    for (uint32_t i = 0; i < 65'537U; ++i) {
        (void)pipeline.ingest(
            faultReading(std::nullopt, timestampMs, TemperatureSampleStatus::CrcFault),
            timestampMs);
        ++timestampMs;
    }

    TEST_ASSERT_EQUAL_UINT16(std::numeric_limits<uint16_t>::max(),
...
void test_recovery_progress_count_saturates_instead_of_wrapping() {
    ...
    for (uint32_t i = 0; i < 65'537U; ++i) {
        (void)pipeline.ingest(okReading(std::nullopt, timestampMs, 20.0), timestampMs);
        timestampMs += 100U;
    }
...
void test_identical_input_sequence_yields_identical_snapshot() {
    ...
    for (auto* pipeline : {&first, &second}) {
        ...
        (void)pipeline->ingest(
            faultReading(identity, 4000U, TemperatureSampleStatus::CrcFault), 4000U);
    }
    ...
    TEST_ASSERT_EQUAL_DOUBLE(snapshotA.rawCelsius.value(), snapshotB.rawCelsius.value());
...
int main() {
    ...
    RUN_TEST(test_snapshot_far_in_the_future_without_further_ingest_detects_failed);
    RUN_TEST(test_identity_change_between_two_known_identities_is_identity_mismatch);
    RUN_TEST(test_identity_transition_from_or_to_unknown_is_not_identity_mismatch);
