...
void test_temperature_source_set_fault_with_ok_status_leaves_reading_unchanged() {
    device_platform_test_support::MockTemperatureSource sensor(
        /*identity=*/std::nullopt, /*monotonicTimestampMs=*/0U,
        /*celsius=*/4.0);

    // setFault() mit status == Ok ist ein Aufruferfehler (eine erfolgreiche
    // Messung gehoert zu setReading()); der Mock darf dabei nicht
    // unkontrolliert fehlschlagen, sondern laesst die zuvor gesetzte Probe
    // deterministisch unveraendert.
    sensor.setFault(/*identity=*/std::nullopt, /*monotonicTimestampMs=*/1000U,
                    device_platform::TemperatureSampleStatus::Ok);
    ...
    TEST_ASSERT_TRUE(reading.celsius().has_value());
    TEST_ASSERT_EQUAL_DOUBLE(4.0, reading.celsius().value());
    TEST_ASSERT_EQUAL_UINT64(0U, reading.monotonicTimestampMs());
}
...
int main() {
    ...
    RUN_TEST(
        test_temperature_source_set_fault_with_ok_status_leaves_reading_unchanged);
