...
bool runtimeAndModeUnchanged(const SensorSelectionStateView& before,
    ...
    return result.runtime == before.runtime &&
           result.activeMode == before.activeMode &&
...
void test_fallback_after_timeout_stays_blocked_before_timeout() {
    ...
    auto decision =
        makeDecision(view, SensorPreference::ProductIfAvailableElseAir,
                     ...
                     validSnapshot(), failedSnapshot(), validSnapshot(), 60U);
...
void test_fallback_after_timeout_falls_back_to_air_when_valid() {
    ...
    TEST_ASSERT_TRUE(result.runtime.permission ==
...
void test_fallback_after_timeout_reaches_safe_locked_without_air() {
    ...
    auto decision =
        makeDecision(view, SensorPreference::ProductIfAvailableElseAir,
                     ProductSensorFailurePolicy::FallbackToAirAfterTimeout,
                     ReturnStrategy::AutomaticValidatedReturnToProduct,
                     failedSnapshot(), failedSnapshot(), validSnapshot(), 60U);
    ...
    TEST_ASSERT_TRUE(result.notice->cause ==
                     SensorSelectionDecisionCause::SafeStateEntry);
}
...
void test_continue_with_air_from_user_decision_required_with_single_air_failure_is_invalid_input() {
    ...
    auto decision =
        makeDecision(view, SensorPreference::ProductIfAvailableElseAir,
                     ...
                     ReturnStrategy::ManualReturnToProduct, failedSnapshot(),
...
void test_continue_with_air_in_product_failure_detected_with_simultaneous_failure_is_invalid_input() {
    ...
    auto decision =
        makeDecision(view, SensorPreference::ProductIfAvailableElseAir,
                     ProductSensorFailurePolicy::FallbackToAirAfterTimeout,
                     ReturnStrategy::AutomaticValidatedReturnToProduct,
                     failedSnapshot(), failedSnapshot(), failedSnapshot(), 60U,
                     SensorSelectionUserAction::ContinueWithAir);
...
void test_wait_for_user_continue_with_air_from_user_decision_required() {
    ...
    auto decision =
        makeDecision(view, SensorPreference::ProductIfAvailableElseAir,
                     ...
                     ReturnStrategy::AutomaticValidatedReturnToProduct,
                     validSnapshot(), failedSnapshot(), validSnapshot(),
                     std::nullopt, SensorSelectionUserAction::ContinueWithAir);
...
void test_wait_for_user_continue_with_air_rejected_for_product_required() {
    auto view = makeView(
        ...
        2U);
    auto decision =
        makeDecision(view, SensorPreference::ProductRequired,
                     ProductSensorFailurePolicy::WaitForUser,
                     ReturnStrategy::AutomaticValidatedReturnToProduct,
                     validSnapshot(), failedSnapshot(), validSnapshot(),
                     std::nullopt, SensorSelectionUserAction::ContinueWithAir);
...
void test_wait_for_user_recheck_product_still_invalid_is_no_change() {
    ...
    auto decision =
        makeDecision(view, SensorPreference::ProductIfAvailableElseAir,
                     ...
                     ReturnStrategy::AutomaticValidatedReturnToProduct,
                     validSnapshot(), failedSnapshot(), validSnapshot(),
                     std::nullopt, SensorSelectionUserAction::RecheckProduct);
...
void test_stop_to_safe_state_reaches_safe_locked_without_product_failure_block() {
    ...
    TEST_ASSERT_FALSE(result.persisted->lastDecisionCause ==
                      SensorSelectionDecisionCause::ProductFailureBlock);
}

void test_stop_to_safe_state_stays_locked_when_product_recovers() {
    auto view = makeView(
        SensorSelectionPhase::SafeLocked, SensorPeltierPermission::Blocked,
        RunSensorMode::Product,
        persistedState(SensorSelectionProvenance::InitialSelection,
...
void test_precedence_simultaneous_air_cooling_failure_during_return_validation() {
    ...
    TEST_ASSERT_TRUE(result.notice->cause ==
                     SensorSelectionDecisionCause::SafeStateEntry);
    TEST_ASSERT_FALSE(result.notice->cause ==
...
void test_rearm_ten_thousand_unavailable_evaluations_never_write() {
    ...
    for (std::uint32_t i = 0U; i < 10000U; ++i) {
        ...
        const auto result =
            applySensorSelectionDecision(view, decision, 600U + i);
...
void test_rearm_repeated_incompatible_aborts_at_most_once_per_attempt() {
    ...
    TEST_ASSERT_TRUE(firstAbort.notice->cause ==
                     SensorSelectionDecisionCause::ReturnValidationAborted);
    TEST_ASSERT_TRUE(firstAbort.runtime.phase ==
                     SensorSelectionPhase::AirFallbackActive);
    ...
    TEST_ASSERT_TRUE(second.runtime.phase ==
                     SensorSelectionPhase::AirFallbackActive);
}

void test_rearm_new_evidence_generation_starts_exactly_one_new_attempt() {
    auto view =
        makeView(SensorSelectionPhase::AirFallbackActive,
                 SensorPeltierPermission::Allowed, RunSensorMode::Air,
                 persistedState(
                     SensorSelectionProvenance::FallbackActive,
                     SensorSelectionDecisionCause::ReturnValidationAborted, 3U),
                 3U);
    view.runtime.returnValidation.lastObservedProfileRevision =
        7U;  // vorheriger Versuch
    ...
    auto decision =
        makeDecision(view, SensorPreference::ProductIfAvailableElseAir,
                     ...
                     ReturnStrategy::AutomaticValidatedReturnToProduct,
                     validSnapshot(), validSnapshot(), validSnapshot());
    decision.plausibility.thermalCompatibility.profileRevision =
        8U;  // neue Generation
...
void test_rearm_compatible_after_full_stability_returns_exactly_once() {
    ...
    decision.plausibility.thermalCompatibility.status =
        ThermalCompatibility::Compatible;
    decision.plausibility.thermalCompatibility.profileRevision = 7U;
...
void test_rearm_product_failure_during_return_validation_needs_new_recognition() {
    ...
    TEST_ASSERT_TRUE(aborted.status ==
                     SensorSelectionApplyStatus::AppliedPersistentCandidate);
    TEST_ASSERT_TRUE(aborted.runtime.phase ==
                     SensorSelectionPhase::AirFallbackActive);
    TEST_ASSERT_TRUE(
    ...
    SensorSelectionStateView stillAirValid = makeView(
        aborted.runtime.phase, aborted.runtime.permission, aborted.activeMode,
        aborted.persisted, aborted.resultingRunRevision);
...
void test_rearm_invalid_to_valid_product_cycle_rearms_without_profile_change() {
    ...
    SensorSelectionStateView recovered = makeView(
        aborted.runtime.phase, aborted.runtime.permission, aborted.activeMode,
        aborted.persisted, aborted.resultingRunRevision);
    recovered.runtime = aborted.runtime;
    auto recoveryDecision =
        makeDecision(recovered, SensorPreference::ProductIfAvailableElseAir,
                     ProductSensorFailurePolicy::FallbackToAirAfterTimeout,
                     ReturnStrategy::AutomaticValidatedReturnToProduct,
                     validSnapshot(), validSnapshot(), validSnapshot());
    ...
    TEST_ASSERT_TRUE(rearmed.status ==
                     SensorSelectionApplyStatus::AppliedRamOnly);
    TEST_ASSERT_TRUE(rearmed.runtime.phase ==
                     SensorSelectionPhase::ReturnValidationPending);
...
void test_recheck_product_from_disallowed_state_is_rejected() {
    auto view = makeView(
        SensorSelectionPhase::NormalAir, SensorPeltierPermission::Allowed,
        RunSensorMode::Air,
        persistedState(SensorSelectionProvenance::InitialSelection,
                       SensorSelectionDecisionCause::StartSelection, 1U),
        1U);
    auto decision =
        makeDecision(view, SensorPreference::AirOnly,
                     ProductSensorFailurePolicy::FallbackToAirAfterTimeout,
                     ReturnStrategy::RemainOnAirUntilEnd, validSnapshot(),
...
void test_stale_decision_on_changed_phase_leaves_state_untouched() {
    ...
    auto decision =
        makeDecision(expected, SensorPreference::ProductIfAvailableElseAir,
                     ProductSensorFailurePolicy::FallbackToAirAfterTimeout,
                     ReturnStrategy::AutomaticValidatedReturnToProduct,
                     validSnapshot(), validSnapshot(), validSnapshot());
    ...
    TEST_ASSERT_TRUE(result.status ==
                     SensorSelectionApplyStatus::StaleDecision);
...
void test_invalid_decision_on_unreachable_runtime_combination() {
    auto view = makeView(
        SensorSelectionPhase::SafeLocked, SensorPeltierPermission::Allowed,
        RunSensorMode::Product,
        persistedState(SensorSelectionProvenance::InitialSelection,
                       SensorSelectionDecisionCause::SafeStateEntry, 2U),
        2U);
...
void test_invalid_context_for_no_active_run_and_restart_revalidation() {
    for (const auto phase :
         {SensorSelectionPhase::NoActiveRun,
          SensorSelectionPhase::RestartRevalidationPending}) {
        auto view = makeView(phase, SensorPeltierPermission::Blocked,
        ...
        TEST_ASSERT_TRUE(result.status ==
                         SensorSelectionApplyStatus::InvalidContext);
        TEST_ASSERT_TRUE(runtimeAndModeUnchanged(view, result));
    }
}
...
void test_time_went_backwards_rejects_without_state_change() {
    ...
    TEST_ASSERT_TRUE(result.status ==
                     SensorSelectionApplyStatus::TimeWentBackwards);
    TEST_ASSERT_TRUE(runtimeAndModeUnchanged(view, result));
}

void test_capacity_reached_rejects_without_state_change() {
    auto view =
        makeView(SensorSelectionPhase::NormalProduct,
    ...
    TEST_ASSERT_TRUE(result.status ==
                     SensorSelectionApplyStatus::CapacityReached);
    TEST_ASSERT_TRUE(runtimeAndModeUnchanged(view, result));
}
...
void test_checked_fallback_delay_overflow_is_invalid_context() {
    ...
    auto decision =
        makeDecision(view, SensorPreference::ProductIfAvailableElseAir,
                     ProductSensorFailurePolicy::FallbackToAirAfterTimeout,
                     ReturnStrategy::AutomaticValidatedReturnToProduct,
                     validSnapshot(), failedSnapshot(), validSnapshot(), 1U);
...
void test_thermal_evidence_future_timestamp_is_invalid_context() {
    ...
    decision.plausibility.thermalCompatibility.evaluatedAtMonotonicMillis =
        200U;
...
void test_return_to_product_blocked_by_single_cooling_sensor_failure() {
    ...
    auto decision =
        makeDecision(view, SensorPreference::ProductIfAvailableElseAir,
                     ProductSensorFailurePolicy::FallbackToAirAfterTimeout,
                     ReturnStrategy::ManualReturnToProduct, validSnapshot(),
                     validSnapshot(), failedSnapshot(), std::nullopt,
                     SensorSelectionUserAction::ReturnToProduct);
...
void test_recheck_product_in_product_failure_detected_blocked_by_single_air_failure() {
    ...
    auto decision =
        makeDecision(view, SensorPreference::ProductIfAvailableElseAir,
                     ProductSensorFailurePolicy::FallbackToAirAfterTimeout,
                     ReturnStrategy::AutomaticValidatedReturnToProduct,
                     failedSnapshot(), validSnapshot(), validSnapshot(), 60U,
                     SensorSelectionUserAction::RecheckProduct);
...
void test_malformed_state_unknown_control_direction_never_grants_return() {
    ...
    auto decision =
        makeDecision(view, SensorPreference::ProductIfAvailableElseAir,
                     ...
                     ReturnStrategy::AutomaticValidatedReturnToProduct,
                     validSnapshot(), validSnapshot(), validSnapshot());
    decision.plausibility.direction =
        static_cast<AbstractControlDirection>(99U);
    decision.plausibility.controlDemandAgeMs = 10U;
    decision.plausibility.thermalCompatibility.status =
        ThermalCompatibility::Compatible;
    decision.plausibility.thermalCompatibility.profileRevision = 7U;
...
int main() {
    ...
    RUN_TEST(
        test_wait_for_user_continue_with_air_rejected_for_product_required);
    ...
    RUN_TEST(
        test_stop_to_safe_state_reaches_safe_locked_without_product_failure_block);
    RUN_TEST(test_stop_to_safe_state_stays_locked_when_product_recovers);
    RUN_TEST(
        test_precedence_simultaneous_air_cooling_failure_during_return_validation);
    ...
    RUN_TEST(
        test_rearm_product_failure_during_return_validation_needs_new_recognition);
    RUN_TEST(
        test_rearm_invalid_to_valid_product_cycle_rearms_without_profile_change);
    RUN_TEST(
        test_recheck_product_from_air_fallback_with_incomplete_evidence_is_ram_only);
    ...
    RUN_TEST(
        test_stale_decision_on_changed_run_revision_leaves_state_untouched);
    ...
    RUN_TEST(
        test_thermal_evidence_zero_profile_revision_with_compatible_is_invalid_context);
    ...
    RUN_TEST(
        test_malformed_state_missing_active_mode_on_mode_change_is_invalid_decision);
    RUN_TEST(
        test_malformed_state_unknown_control_direction_never_grants_return);
    RUN_TEST(
