...
```text
Arbeite im Repository `ManuEngineer/ESP32-Fermentationsschrank` ausschliesslich an Issue #15:
"[E1.4] Laufkommandos, Meldungen und Bedienaktionen implementieren"
...
6. Verbindlicher Scope von Issue #15
   - Startzusammenfassung und bestaetigter Start
   - Stopoptionen: Abbruch/Aus, Abbruch/Kuehlen, Zurueck
   - Abschluss und Quittierung
   - `Quittieren` getrennt von `Fehler zuruecksetzen`
   - Meldungsprioritaeten und akustische Ereignisse als fachliche Kommandos
   - Zieltemperatur- und Restdaueranpassung mit Vorschau/Bestaetigung
   - gleichzeitige Bedienaktionen konfliktfrei behandeln

7. Akzeptanzkriterien
   - Kommandos sind atomar und zustandsabhaengig validiert
   - Quittierung loest keine Verriegelung
   - Stoppen fuehrt nie zu verbotener Aktorwiederherstellung
   - Laufanpassungen erzeugen eine protokollierbare Revision
   - konfliktierende Kommandos liefern eindeutige Ergebnisse

8. Issue-spezifische Tests und Nachweise
   - alle Stopoptionen
   - Quittierung und Reset
   - Display-/Web-Konflikt
...
11. Pull Request
    ...
    - `Closes #15` nur bei vollstaendig erfuellter Definition of Done; sonst `Refs #15`.
...
Kommandos, Ergebnisse, Tests und Dokumentation abgeschlossen.
