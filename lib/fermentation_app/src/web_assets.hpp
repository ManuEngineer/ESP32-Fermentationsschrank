...
async function request(path,options={}){const h=Object.assign({},options.headers||{});if(csrf)h['X-CSRF-Token']=csrf;if(options.mutation){h['X-UI-Mutation-Seq']=String(nextSeq);options.body=JSON.stringify(options.body||{});h['Content-Type']='application/json'}const r=await fetch(path,Object.assign({},options,{headers:h}));const d=await r.json().catch(()=>({}));if(d.csrfToken)csrf=d.csrfToken;if(d.nextMutationSeq)sequence(d);if(options.mutation&&r.ok)nextSeq++;if(!r.ok)throw new Error(d.message||d.code||String(r.status));return d;}
function render(data){if(data.csrfToken)csrf=data.csrfToken;sequence(data);expectedRevision=data.expectedUserConfigurationRevision||null;$('state').textContent=data.ready?'ready':'unavailable';$('overview').textContent=JSON.stringify({networkMode:data.networkMode,processState:data.processState,activeRunId:data.activeRunId,temperatures:data.temperatures},null,2);const s=$('networkMode');s.replaceChildren(...(data.selectableModes||['AP_ONLY','HOME_WIFI']).map(m=>{const o=document.createElement('option');o.value=m;o.textContent=m;return o;}));s.value=data.networkMode==='UNSELECTED'?'AP_ONLY':data.networkMode;if(typeof data.webPasswordEnabled==='boolean')$('passwordEnabled').checked=data.webPasswordEnabled;$('alerts').textContent='';}
