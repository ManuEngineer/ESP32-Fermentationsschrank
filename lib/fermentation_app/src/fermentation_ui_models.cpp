...
bool equalRuntimeMessage(const RuntimeMessage& left,
    ...
    return left.id == right.id && left.code == right.code &&
           left.messageClass == right.messageClass && left.priority == right.priority &&
           left.trigger == right.trigger && left.monotonicMillis == right.monotonicMillis &&
           left.active == right.active && left.acknowledged == right.acknowledged &&
           left.resolved == right.resolved && left.decisionRequired == right.decisionRequired &&
           left.acousticMuted == right.acousticMuted &&
           left.acousticIntent == right.acousticIntent && left.runRevision == right.runRevision &&
           left.stateSequence == right.stateSequence && left.faultRevision == right.faultRevision &&
...
bool equalQuality(const device_platform::SensorQualitySnapshot& left,
    ...
    return left.identity == right.identity && left.quality == right.quality &&
           left.rawCelsius == right.rawCelsius && left.correctedCelsius == right.correctedCelsius &&
           left.filteredCelsius == right.filteredCelsius && left.appliedOffset == right.appliedOffset &&
...
bool equalPresentation(const PresentationState& left,
    ...
    return left.faultCode == right.faultCode && left.acknowledged == right.acknowledged &&
           left.resetCause == right.resetCause &&
           left.applicationAllocationFailure == right.applicationAllocationFailure;
}

}  // namespace

bool equalFermentationUiSemanticSnapshot(const FermentationUiSnapshot& left,
                                         const FermentationUiSnapshot& right) noexcept {
    if (left.revisions.expectedStateSequence != right.revisions.expectedStateSequence ||
        left.revisions.expectedRunRevision != right.revisions.expectedRunRevision ||
        left.revisions.expectedMessageRevision != right.revisions.expectedMessageRevision ||
        left.revisions.expectedFaultRevision != right.revisions.expectedFaultRevision ||
        ...
        left.home.mode != right.home.mode || left.home.processState != right.home.processState ||
        left.home.activeRunId != right.home.activeRunId ||
        !equalEffectiveValues(left.home.effectiveValues, right.home.effectiveValues) ||
        ...
        left.messages.size() != right.messages.size() || left.recovery.mode != right.recovery.mode ||
        left.recovery.canonicalRecoveryDisposition != right.recovery.canonicalRecoveryDisposition ||
        left.recovery.persistenceLoadStatus != right.recovery.persistenceLoadStatus ||
        left.recovery.coordinatorState != right.recovery.coordinatorState ||
        !equalPresentation(left.status.presentation, right.status.presentation) ||
        left.status.ready != right.status.ready || left.service.available != right.service.available ||
        left.service.confirmationRequired != right.service.confirmationRequired ||
        left.service.serviceAuthorizationRequired != right.service.serviceAuthorizationRequired ||
    ...
    for (std::size_t i = 0U; i < left.temperatures.size(); ++i) {
        if (left.temperatures[i].role != right.temperatures[i].role ||
            left.temperatures[i].valueCelsius != right.temperatures[i].valueCelsius ||
            !equalQuality(left.temperatures[i].quality, right.temperatures[i].quality)) {
            return false;
        }
    }
    for (std::size_t i = 0U; i < left.messages.size(); ++i) {
        if (!equalRuntimeMessage(left.messages[i].message, right.messages[i].message)) {
            return false;
        }
    }
    return true;
...
device_platform::UiRefreshRevision FermentationUiRefreshRevisionTracker::publish(
