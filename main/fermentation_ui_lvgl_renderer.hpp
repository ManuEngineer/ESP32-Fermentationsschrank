...
class ProductiveLvglRenderer final {
   public:
    ...
    //
    // Serialized against the LVGL touch input callback (which runs on the
    // separate LVGL task esp_lvgl_port owns) via the existing
    // lvgl_port_lock()/unlock(); this method never introduces a second,
    // renderer-owned threading primitive. Before the LVGL task exists
    // (initialize() not yet called), the update applies directly.
    ...
    // The renderer-owned raw-touch-to-display-coordinate state a caller
    // needs to drive the existing #26 target/interaction path itself; see
    // fermentation_ui_renderer.hpp's targetAt()/routePress(). This renderer
    // never calls those itself - it only reports calibrated touch state, so
    // FermentationTouchWorkspace is read/mutated exclusively from the
    // caller's own task, never from the LVGL task.
    struct TouchPollResult {
        // The current held state, for deriving render()'s pressedTarget
        // (so visible press feedback clears again on release).
        bool contactHeld{false};
        // Set iff contactHeld; the calibrated, display-bounds-clamped
        // coordinate of the current contact.
        std::optional<TouchPoint> point;
        // True exactly once per new press (a release-to-held transition);
        // consumed by this call. A caller uses this to call routePress()
        // exactly once per press, not on every tick a contact is held.
        bool freshPressEdge{false};
    };

    // Reads and consumes the touch state the LVGL callback observed since
    // the last call. Serialized against that callback the same way as
    // setTouchCalibration(); on a lock timeout, returns a default
    // (no-event) result rather than fabricating a release, so a real held
    // press is never silently dropped by a transient lock contention.
    [[nodiscard]] TouchPollResult pollTouch();
